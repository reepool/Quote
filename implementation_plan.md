...
- **估值历史建议采用混合模式**：
  ...
  - `2026-07-18` 公司画像目标统一调整为“免费结构化数据优先、官方披露按需复核、人工治理兜底、LLM 延后”。东方财富主营构成负责报告期产品/行业/地区收入、成本、利润及占比，同花顺主营介绍提供当前复核上下文；结构化源只写 candidate evidence/segment，不自动推断上下游、客户供应商、价值链角色或公司商品暴露。OpenSpec `build-a-share-business-profile-evidence-pipeline` 已按该目标重写。
  ...
  - `2026-07-18` 已预留默认关闭的 OpenAI-compatible selected-section 接口：输入页码、标题、文本和 hash，输出 `business_profile_llm_report.v1` 结构化 candidate；发送前复核唯一 section id、正页码和规范文本 hash，响应必须绑定版本化业务事实目录并通过字段、值类型、单位、模型/prompt/schema/request/response lineage 和证据引用校验。当前无本地模型、无 scheduler、无数据库 writer、无 DCF 接入，后续须另行完成质量、token、时延和吞吐评估。
  - `2026-07-18` 已固定未来本地 LLM benchmark 合同：开发集、冻结 holdout 和 challenge set 按公司隔离，双人标注和第三人裁决；分别考核字段、数值单位期间、明确关系、证据定位、幻觉、稳定性、硬件和吞吐。任何无证据关系、虚构数字、目录外字段或 approved/DCF 泄漏均直接否决。只有独立基准通过后才能另开 `promote-business-profile-local-llm-extraction`，且首阶段仍只允许 bounded candidate writer。
