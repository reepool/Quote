...
- **估值历史建议采用混合模式**：
  ...
  - `2026-07-16` DCF 公司业务画像治理基础层已推进：新增证据、分部、经营事实、价值链角色和商品暴露规范化存储；按审批、证据完整性、可得日和有效期构建 `business_profile_context`，公司 approved 暴露可在 DCF 周期诊断中覆盖同商品/角色行业默认，并新增画像、历史、商品暴露和复核队列只读 API。当前生产 approved 公司画像覆盖仍为空或有限。
  - `2026-07-18` 公司画像当时统一调整为“免费结构化数据优先、官方披露按需复核、人工治理兜底、LLM 延后”。东方财富主营构成负责报告期产品/行业/地区收入、成本、利润及占比，同花顺主营介绍提供当前复核上下文；结构化源只写 candidate evidence/segment，不自动推断上下游、客户供应商、价值链角色或公司商品暴露。OpenSpec `build-a-share-business-profile-evidence-pipeline` 已按该目标重写；公共 LLM 网关在 `2026-07-20` 完成后由后续专项承接语义候选抽取。
  ...
  - `2026-07-16` 业务画像 PDF 原件归档第一版已实现：复用 `financial_source_files` 并新增显式 `source_tier / supersedes_source_file_id`；同公告同内容短路，修订稿和附件内容变化建立 supersession，有限批次支持原子 checkpoint 中断续跑。`2026-07-17` archive parser 升级为 v2，归档布局改为 `research_config.modules.business_profile_evidence.archive` 配置驱动，默认主目录 `data/filings/business_profile`、子目录 `{year}/{market}`、文件名 `{instrument_id}_{period_label}_{announcement_id}_{content_hash}.pdf`，例如 `600839_SH_2025Q4_<announcement>_<sha256>.pdf`。模板必须保留 `{content_hash}`，防止同报告期修订稿覆盖；布局变更只影响新文件，既有 manifest 原件不自动迁移。当时尚未执行生产归档，后续交易所备源、page artifact 和候选链已分别继续实现
  ...
  - `2026-07-17` PDF artifact 升级为 v2 parser diagnostics：文档层区分加密、损坏和不支持，页文本层识别 glyph 解码异常并只把字段相关页面送入 OCR 队列；表格失败、模板不支持和原文未披露使用不同阶段 outcome，`not_disclosed` 必须有已成功解析章节及证据页，不能由空文本或解析失败推断。诊断阈值进入 parameter hash，并由只读 artifact CLI 输出结构化诊断。
  - `2026-07-17` 原五家公司 PDF parser benchmark 选择器和只读 CLI 作为官方文档诊断工具保留，但不再是结构化业务画像上线前置。30 家行业样本改用于东方财富/同花顺结构化接口 live probe；正式 PDF 仅对字段冲突、高材料性审批、专项经营字段和未来 LLM 关键 section 按需获取。
  - `2026-07-17` 增加配置驱动的交易所官方备源发现。CNInfo 仅在失败、降级、身份缺失或明确报告查询为空时切换到 instrument 对应交易所；SSE/SZSE 已通过单公司单页 live 响应验证，BSE adapter 因生产网络重定向循环保持禁用。候选、来源诊断和归档 manifest 均保留真实 source tier，不再把交易所附件记为 CNInfo。
  - `2026-07-17` 增加交易所备源只读 live 验证 CLI：强制 canonical instrument、日期范围、最多 5 页和最多 20 家硬上限，默认 1 页/10 家；支持 chain/backup 两种模式，输出每个实际来源的页数、公告数、候选数和错误，不下载附件、不写生产状态。配合单元测试完成标题、全文/摘要、fallback、重复内容、更正跨来源 lineage 和中断续跑验收，OpenSpec `2.6` 完成。
  - `2026-07-17` benchmark review 门槛已收紧：全局 readiness 强制六个首期行业全部存在并各自达到 5 家，空库、缺行业或单行业不能误报 ready；证据 ID 必须在 `financials.db` 匹配同公司、官方主/备来源层、业务画像 schema、有效状态、公告 ID、归档路径和 SHA-256，伪造或跨公司 ID 不计入覆盖；`table_parse_failure` 必须携带至少一个证据页。
  - `2026-07-17` 新增 benchmark 官方文档 probe：从 30 家选择结果按行业、标的和数量上限执行 CNInfo `searchkey/category/date/page` 受控发现，默认仅返回 metadata；只有显式指定非生产 `download_root` 才下载内容寻址 PDF 并运行页级 diagnostics，始终 `dry_run=true`，不更新公告水位、生产 manifest 或画像事实。日期范围必须成对提供，生产 `data/filings/business_profile` 路径被显式拒绝，非 PDF 和 `parse_failed` 均 fail closed。
  - `2026-07-17` CNInfo live probe 已覆盖既定 30 家 metadata，三市场均可解析身份且未发现无正式报告标的；真实更正/补充后全文已在 `000983.SZ / 000506.SZ / 000968.SZ / 000629.SZ` 识别。在线样本同时修复三类全文误报：年度报告治理制度、年度报告更正公告、年度报告业绩说明会/预告。更正公告保留为 `*_correction_notice` 治理证据，但不再伪装成修订后全文或触发全文 supersession；分类与报告期推断共用同一年度标题正则，避免把无有效年度格式的半年报按公告日归档。
  - `2026-07-17` 六个首批行业各 1 份 2025 年正式年报已在 `/tmp` 完成 diagnostics-only 基线，共 `1112` 页、`308` 个业务标题命中，全部页面可提取原生文本，无下载、PDF、页树、乱码或 OCR blocker；低文本页共 `4` 页且均未命中业务目标。该结果只证明当前六份 PDF 的原生文本链可用，不代表 30 家正式归档、复杂/跨页表验证或多元业务人工标签完成，OpenSpec `1.2` 继续保持未完成。
  - `2026-07-16` 业务画像 PDF 派生 artifact 基础已实现：`pypdf` 验证和逐页原生文本、密度、标题索引、文本/页/文档哈希写入确定性 gzip JSON，目录固定为 `derived/tables/ocr/{version}/{source_hash}_{parameter_hash}.json.gz`；低文本页仅在业务目标页范围内进入 `ocr_required`，并提供单文件本地命令。artifact 内容身份排除本地路径和单一 manifest ID，不同参数通过 parameter hash 隔离；本地既有官方 PDF 的 diagnostics-only smoke 为 `304` 页、`302` 页原生文本、`2` 页低文本、`41` 个标题命中且无提取异常。当前尚未选择表格解析器或 OCR worker；分层失败诊断合同已于 `2026-07-17` 补齐，字段事实层完整状态集仍待后续解析任务接入
  - `2026-07-17` 业务画像事实目录 `business_profile_facts.2026.1` 已实现：29 个字段覆盖分部、经营事实、价值链角色和商品暴露，逐字段固定语义、数值类型、规范单位或枚举允许值、材料性、候选/勾稽/审核策略和 DCF 资格。配置加载执行 fail-closed 校验，所有 DCF eligible 字段必须人工审核；单位成本、储量、合同、套保和商品敏感性等敏感字段只允许基于明确披露生成 candidate，不得从行业假设自动推导，也不得自动批准。目录发布日期与可处理文档日期分离，当前版本发布于 `2026-07-17`，适用于 `2021-01-01` 起的历史报告。该目录仅是字段契约，尚不表示生产数据覆盖或 promotion gate 已通过。
  - `2026-07-17` 业务画像披露模板目录 `business_profile_disclosure_templates.2026.1` 已实现：通用定期报告模板与煤炭、有色/固体矿产、钢铁、石油石化、基础化工、建筑材料 6 个行业模板按规则版本、生效日、交易所、板块、全文类型和行业组选择，章节别名和表头签名均引用已治理 `field_id`。解析签名与市场规则作用域分离，选择结果返回实际匹配的交易所、板块、文档类型、规则版本和 `csrc_common_rule / exchange_industry_rule / observed_parser_pattern` 权威类型；本地主数据 `main_board / star_market` 及中文板块名均先规范化。合计、小计、抵销、未分配和不适用行保留并标注行角色，用于勾稽而不是直接删除。行业模板只收窄候选页面/表格，不从申万分类直接生成公司事实。
  - `2026-07-18` 业务画像产品目录升级为 `business_profile_products.2026.2`：保留 40 个产品、48 条精确别名和 44 条 candidate-only 商品 master 映射，删除上下文关键词、排除词和值链角色依赖。只有明确产品收入、原材料、能源、库存或套保事实才能筛选相应映射，未知和歧义标签保持待审，不写公司级 exposure。
  - `2026-07-18` 已删除未进入生产的 `business_profile_value_chain_rules` 词法推断代码和目录；`business_profile_units.2026.1` 保留并增加 `business_profile_facts.2026.1` 版本锁。新增免费结构化 provider 和 candidate writer，单源失败可降级；生产 transport 使用项目受控 HTTP session，timeout、pacing、retry 和 backoff 实际生效。evidence 按 payload 快照幂等，segment 按来源行、parser 和产品目录版本增量治理，新增报告期不复制未变化历史行，目录升级可重放并建立 supersession；聚合源使用首次观测日作为保守可得日。
  - `2026-07-18` 已预留默认关闭的 OpenAI-compatible selected-section 接口：输入页码、标题、文本和 hash，输出 `business_profile_llm_report.v1` 结构化 candidate；发送前复核唯一 section id、正页码和规范文本 hash，响应必须绑定版本化业务事实目录并通过字段、值类型、单位、模型/prompt/schema/request/response lineage 和证据引用校验。当时无可用模型、无 scheduler、无数据库 writer、无 DCF 接入；`2026-07-20` 公共 LLM 网关完成后，画像适配器已迁移到公共调用边界，其余生产闭环由新专项承接。
  - `2026-07-18` 已固定首版 LLM benchmark 合同：开发集、冻结 holdout 和 challenge set 按公司隔离，双人标注和第三人裁决；分别考核字段、数值单位期间、明确关系、证据定位、幻觉、稳定性和吞吐。任何无证据关系、虚构数字、目录外字段或 approved/DCF 泄漏均直接否决。`2026-07-20` 该合同升级为 provider/实际 model/prompt/schema/selector 统一身份，并由 `integrate-llm-business-profile-supply-chain` 承接，首阶段仍只允许 bounded candidate writer。
  - `2026-07-18` 新增产品标签治理闭环：候选产品分部按最新来源行版本生成未匹配/歧义审计，保留原标签、行业、报告期、来源和目录版本；精确别名升级要求期望旧版本、操作人、理由和正式证据引用，只输出新目录及 promotion manifest，禁止原地覆盖或运行时自动学习。
  - `2026-07-18` 免费结构化业务画像新增统一 bounded sync service/CLI：使用申万时点行业和上市生命周期选取 A 股范围，支持来源/行业/公司 scope、配置数量和时长硬上限、dry-run、显式 candidate 写入开关、公司×来源 checkpoint/resume。候选写入强制保存内容寻址 gzip raw cache 和 run manifest，evidence 只存可校验引用；报告汇总来源健康、耗时、行数、期间、payload 变化、别名命中和 DCF 零泄漏。当前模块和后续接入的统一 scheduler job 均保持 `enabled=false`；六行业分层 live probe 和临时库回补已通过，下一门槛是正式报告 99% precision、目录 promotion 与单行业 production candidate pilot。
  - `2026-07-18` 六行业各 5 家分层只读 pilot 已完成，样本覆盖 `BSE 4 / SSE 8 / SZSE 18` 和四个上市年代；两个来源均 `30/30` 成功，总耗时约 `32s`，保存 60 个 `/tmp` 内容寻址 raw snapshots。东方财富 `24/30` 达到 200 行疑似上限，产品精确别名覆盖仅 `61/2797=2.18%`，另有少量非标准报告日期；因此来源链可用，但正式报告精度核对、目录 promotion 和生产 candidate 写入仍未通过。
  - `2026-07-18` 同一 30 家样本已在全新 `/tmp` research DB 完成三阶段 candidate backfill 验收：首轮真实写入后按公司×来源 checkpoint 恢复 1 个瞬时失败来源，再做全量重放；最终 `60` evidence、`5,554` candidate segments，全量重放新增 `0/0`，approved/value-chain/company exposure 均为 `0`，`quick_check=ok`。OpenSpec `6.4` 完成，但正式报告 `99%` precision 和单行业 production pilot 仍未开始。
  - `2026-07-18` bounded sync 审查修复完成：来源证券代码错配 fail closed，请求/retry 使用剩余 deadline；raw manifest 成功后才允许 candidate 写入和 checkpoint 完成；ingestion run 在 manifest/运行异常下仍写终态；DCF 零泄漏改为 approved/价值链角色/公司商品暴露的运行前后数据库差值。标签审计先取最新来源行，超限返回 incomplete/非零退出；目录与 promotion manifest 不可覆盖，manifest 发布失败回滚目录输出。
  - `2026-07-18` 业务画像人工审核闭环已实现并完成审查修复：`business_profile_review_audit` 追加式记录 approve/reject/supersede 决定并由 SQLite trigger 禁止更新和删除；CLI 写入要求字面量 operator switch、审核人、理由、预期状态和预期更新时间，queue/audit 使用只读 SQLite 连接。非 evidence 记录只有在同公司 evidence 已批准后才能批准，非官方 evidence 必须留下复核依据；结构化 upsert 在 SQLite conflict 条件中保护终态，避免并发同步降级审核结果。正式报告产品标签复核包只接受 `financials.db` 中同公司、同报告期、官方来源层、有效且未被替代、归档哈希一致的完整年报/半年报 manifest，人工结论必须指定文档 hash 和页码；排除原因受治理且默认排除率不超过 5%。统计 gate 使用双侧 95% Wilson 下界，99% 下界在零错误时至少需要 381 条有效样本；只接受唯一产品映射并去除同公司、同期间、同规范标签和同映射的跨来源明显重复，不引入按公司或行业的一刀切去重。先前 `/tmp` 六份 PDF 目录产生的 1 条候选没有 manifest 绑定，只能作为工具诊断；当前 51 条材料性候选均已绑定生产合格 manifest，但仍未达到 381 条且缺有色、建材候选，`6.3` 保持未完成，生产同步和 scheduler 继续关闭。
  - `2026-07-18` 结构化业务画像已接入唯一的 `business_profile_structured_sync` 周度 scheduler 入口，复用现有 bounded service，不拆分来源或行业任务。job、业务画像模块和免费结构化源均默认关闭；模块关闭时在 provider 构造前返回，启用后仍受候选写 operator switch、30 家/900 秒配置硬上限、checkpoint/resume 和 candidate-only 约束，且不调用 LLM。`resume=true` 在 checkpoint 不存在时创建新批次，已有且 scope 一致时才恢复；服务和调度测试已覆盖首次启动、单源降级、未变化、中断、恢复、disabled 与非到期情形。OpenSpec `5.5/5.6` 完成，但 `6.3` 正式报告精度和 `6.5` 单行业生产 pilot 仍未完成，因此不得开启生产调度。
  - `2026-07-18` 产品标签验证新增只读 precision-corpus readiness 审计，分别输出材料性唯一映射候选、公司/报告期、完整年报/半年报 manifest 绑定、六行业文档覆盖及短缺 blocker；未匹配/歧义标签审计增加材料性行数、材料性公司数和最大收入占比，只用于人工复核排序。初始生产库候选/正式 manifest 均为 0；30 家隔离库排除歧义映射和跨来源明显重复后为 `51/381` 条、5 家公司、51 个公司/报告期，候选层缺有色与建材。下一阶段先归档高材料性标签对应正式报告、受控升级精确别名并重放隔离候选，再补足六行业文档覆盖和全局 381 条正式报告绑定样本；不设置每行业统一最低样本数，不得直接开启生产同步。
  - `2026-07-18` 新增材料性 precision 候选驱动的正式报告编排 service/CLI，复用 CNInfo 主源、交易所官方备源、不可变归档、`financial_source_files` manifest 和按公司 checkpoint。默认模式只读发现元数据；写入必须显式传入 `BUSINESS_PROFILE_OFFICIAL_ARCHIVE_WRITE`，每次最多 20 家、每家 50 份、10 页，并实测业务画像候选表前后差值为 0。首个生产归档试点已将 `000717.SZ` 的 2025 年完整年报写入规范目录并登记 manifest，SHA-256 校验、父子 ingestion lineage 和 checkpoint 清理均通过；限定 2025 年的隔离候选目前为 2 行、正式报告绑定 1 行，仍远低于 381 条，OpenSpec `6.3` 不得勾选。
  - `2026-07-18` 正式报告证据基线扩展到隔离库全部 5 家、51 个候选报告期：巨潮扫描按实际上游上限 30 行分页，宽关键词窗口缺期时按单一年份和法定披露窗口重试主源，之后才使用对应交易所备源；官方“年度半年度报告/半年报”标题已纳入严格完整报告分类。最终发现和 manifest 绑定均为 `51/51`，缺失期间、文件失败、SHA-256 校验错误和残留 checkpoint 均为 0，候选表写入差值为 0。readiness 仍为 `not_ready`，因为全局还差 330 条且缺有色、建材材料性唯一映射候选；下一步是官方页码证据驱动的精确别名 promotion 和隔离重放，不开启生产 scheduler 或 `6.5`。
  - `2026-07-19` 正式报告编排新增显式 `catalog_issues` 目标范围，解决“只有已精确映射标签才能触发报告归档、未匹配标签却无法取得 promotion 证据”的闭环缺口。该范围只读取最新版、candidate、达到材料性阈值且诊断为 `alias_not_found / ambiguous_product_alias` 的产品行；只保留可由完整年报/半年报承载的 `06-30 / 12-31` 合法期间，同一事实跨来源合并诊断并保留收入占比最高的代表行。默认仍为 `precision_exact`，不会自动新增别名、批准候选、生成公司商品暴露或进入 DCF。30 家隔离库识别出 `2,096` 条目录问题、`30` 家公司和 `486` 个公司/报告期；对建材 `000012.SZ`、有色 `000506.SZ` 的 2025 年报只读探测均由巨潮主源命中，候选表差值为 0。该能力只打通 `6.3` 的证据获取前置，381 条 manifest-bound 精度样本和 99% 人工比较仍未完成。
  - `2026-07-19` 精确别名 promotion 改为强制结构化官方证据：`promote-alias` 必须读取 `business_profile_product_alias_official_evidence.v1` JSON 和 `financials.db`，并在写新目录前验证同公司/报告期的活动官方完整报告 manifest、未被替代状态、本地 SHA-256、PDF 实际页数、引用页原生文本及 hash、正式标签在至少一个引用页出现、产品/行业、审核人、带时区审核时间和理由，同时要求审核时间不晚于 promotion 时间。相对归档路径按显式基准目录解析，不依赖进程工作目录；验证摘要和 hash 进入不可变 promotion manifest，不复制本地归档路径或页全文。扫描件或文本解码异常页保持 fail closed，留待受控 OCR artifact 处理；旧的任意字符串 `evidence-reference` 路径已删除。当前只是治理能力完成，尚未替代人工审阅或新增生产别名。
  - `2026-07-20` 目录问题人工操作链已补齐：`research_business_profile_precision_review.py export-catalog-issues` 将最新版材料性未匹配/歧义 candidate、来源身份、行业、收入占比和活动官方完整报告导出为 `business_profile_product_catalog_issue_review.v1`，每行及整包均有稳定 hash；人工只能逐行填写 `promote_alias / defer / exclude`，`prepare-promotion-evidence` 仅为 promotion 决定生成受控官方证据 JSON，并校验候选快照、正式标签与来源标签、报告 ID/hash/期间、页码、产品、行业及人工审核字段。审核源快照只保留可移植的报告身份字段并规范化文档顺序，不把本机路径和 URL 纳入编号；promotion 端重新计算快照 hash，并核对来源记录、来源行、标签和问题类型，防止 lineage 被替换。生成证据继续由目录 promotion 执行 PDF 原文与页级 hash 二次门槛；工具不自动选产品、不改目录、不审批候选。定向业务画像与 DCF 回归现为 `35 passed`；扩大测试在既有 structured-ingestion 的 `asyncio.to_thread` 测试等待 worker 时达到沙箱超时，与本次同步复核调用链无关。`6.3` 仍未完成，下一步是实际人工复核有色、建材及其他材料性标签，而不是继续增加自动语义规则。
  - `2026-07-20` 公共 LLM 网关完成后，公司画像语义能力进入新的受控集成阶段：新增 `llm_assisted_business_profile_and_supply_chain_requirements.md`，明确只从官方报告关键 section 抽取有精确原文 quote/offset/page/hash 的 candidate；分部和经营事实复用现有治理表，具名客户/供应商使用独立供应链关系模型，模型不得自动解析模糊实体、批准商品暴露或进入 DCF。OpenSpec `integrate-llm-business-profile-supply-chain` 负责公共 profile 配置收口、选段、v2 schema、run audit、candidate writer、供应链审核、冻结基准和有界试点；旧 change 尚未完成的正式报告精度、结构化生产试点和成本端映射任务继续独立保留。
  - `2026-07-21` 项目官方公告获取已统一到 `research.announcements`，公司画像发现和附件下载也已完成迁移：业务 adapter 通过 `AnnouncementAcquisitionService` 提交 `business_profile_evidence:<instrument_id>` source-neutral query，只保留标题/报告期分类；通用层负责 CNInfo/SSE/SZSE/BSE provider 能力、配置化路由、fallback、保守 cursor、selected audit 和受信任附件取回，画像域继续负责不可变归档、manifest、更正、PDF artifact、LLM 选段和事实审批。当前 LLM 专项文档与 OpenSpec 已同步要求复用该边界，不再建设画像域公告 provider、fallback 或下载 transport。
  - `2026-07-20` 继续补齐特殊商品到公司 DCF 的真实接线：approved 公司暴露现在可解析期货连续序列或 `commodity_price_series`，特殊商品上下文不再返回全库最新诊断，只读取公司 mapping 指定序列并按估值日截断。`commodity_cycle_context` 可让单一 approved 收入端特殊商品进入周期模型；原材料/能源成本单腿、多序列或跨期货/特殊商品序列未实现价差解析时阻塞，避免收入端与成本端方向混用。本地基线为特殊商品 `44` 个、序列 `47` 条、观测 `102,053` 条，但 approved 公司暴露、spread definition、exposure mapping 仍均为 `0`。本轮 resolver/DCF/估值、特殊商品主数据和工作簿回归合计 `82 passed`；`6.8` 仍须建立正式成本证据、人工批准映射和价差定义后才能完成。
  ...
  - `2026-05-31` 过去 12 个季度估值历史任务完成后复核：`valuation_history` 同一完整主键无重复，但表内曾并存旧 `252` 交易日窗口 parameter hash 与新 `last_12_quarters` parameter hash。后续 `parameter_hash` 只代表“同一交易日估值结果如何计算”，不得包含 `lookback_days / window_mode / quote_limit_days / write_policy` 等任务窗口或调度参数；这些运行窗口只记录在 `ingestion_runs.metadata_json` 和任务报告里。读路径按当前 canonical hash 过滤，并临时兼容当前配置的 legacy full-history hash，避免迁移前 API/readiness 读不到既有 12q 行；旧 hash 只能在备份后通过显式维护脚本删除或 rewrite 到 canonical hash。
  ...
  - `valuation_history.details_json` 必须保持 compact lineage：落库使用短字段 `v=2` 存储格式，读取 API 自动还原为原长字段结构；保留估值输入来源、关键财务期、可得日、指标分母和 unavailable reason，不在日频行中重复保存完整外部 payload 或大体量 diagnostics。存储维护通过 `scripts/research_valuation_storage_maintenance.py` 先审计 parameter hash 与 `dbstat` 体积，再显式清理旧口径或执行 details compact。
  - `2026-06-01` 已完成既有 `valuation_history.details_json` 压缩与 VACUUM：`2,707,947` 行全部迁移到 compact v2；`details_json` 文本合计约 `7.96GB -> 3.10GB`，平均长度约 `2938 -> 1143` 字符；`valuation.db` 文件约 `12GB -> 5.9GB`，`WAL=0`，`quick_check=ok`。
...
- `full financial statements`
  ...
  - `2026-05-26` 晚间 `financial_disclosure_incremental_sync` 报告中出现 `旧噪声过滤=3` 后复核：3 条并非业绩说明会等噪声，而是 `002731.SZ / 2025-12-31`、`002731.SZ / 2026-03-31`、`688121.SH / 2026-03-31` 的定期报告披露进展/退市风险公告。根因是旧 pending 清理逻辑要求标题能推断出明确报告期，导致无具体期次但仍有效的 `pending_delisting_risk` 被误标为 `filtered_stale_noise`。已修正规则：`pending_delisting_risk / periodic_report_delayed` 不因标题缺显式报告期而被当作旧噪声；并已将误标的 3 条状态恢复为 `pending_delisting_risk`。后续旧噪声过滤仅用于业绩说明会、英文版、图文版等不应触发财报补数的历史 pending。
  - `2026-05-26` 财务公告状态表继续收口：同一 `instrument_id/report_period` 一旦被公告证据、生命周期规则或成功补数解释为非 blocker，维护状态表会删除对应 `local-gap:*` 旧 blocker，避免历史对账状态在库表层面继续干扰当前状态判断。
  - `2026-05-26` 清理 `financial_disclosure_event_state` 中旧逻辑产生的过期维护状态：移除 2026-05-25 对账 run `384` 写入的 `500` 条历史 `blocking_gap/local_core_gap` 状态；该批记录的运行摘要仍保留在 `ingestion_runs`，但不再作为当前维护状态展示。
  - `2026-05-26` 财务收尾测试发现 `financial_disclosure_event_state` 空表仍会在 `research.db` 初始化后残留，违反 `financials.db` 作为财务物理库的约定。已把该表加入研究库财务表清理名单；当 `financials_db_path != db_path` 时，研究库初始化会删除所有 `financial_*` 物理表，财务公告状态继续只在 `financials.db` 中维护。`cninfo_announcement_*` 表仍保留为通用公告审计/股东任务共享表，不按财务表清理。
  - `2026-05-26` 备份任务同步扩展：`database_backup` 不再只备份 `quotes.db`，生产配置已覆盖 `quotes.db / research.db / financials.db / market_data.db`，并通过 `include_extra_data_dbs=true` 自动纳入未来新增的 `data/*.db`；清理策略按每个数据库备份文件模式分别保留 `max_backup_files`。
...
| `shareholder_reconciliation_sync` | 每周六 `12:30` | 股东摘要周期复核与补足。全量读取 `SSE / SZSE / BSE` 活跃股票后与日更相同的归一化内容 hash 和 required scope 对比，只补写变化、缺失或覆盖不完整标的；用于兜底 CNInfo 静默修正、公告漏识别和历史缺口 |
