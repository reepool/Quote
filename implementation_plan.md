...
- **估值历史建议采用混合模式**：
  ...
  - `2026-07-17` 新增五家公司 parser benchmark 分层选择器和只读 CLI。生产库基线已确定性生成 6 个行业共 30 家候选，并覆盖各行业实际存在的沪深北市场；只有带官方 `source_document_ids` 的 verified 文档标签才能满足综合经营、修订稿和 PDF format edge 门槛。当前业务画像 manifest 仍为 0，六个行业均正确返回 `evidence_incomplete`，因此 OpenSpec `1.2` 保持未完成，下一步对 30 家候选执行受控官方报告发现、归档和页级诊断。
