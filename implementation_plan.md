...
- **估值历史建议采用混合模式**：
  ...
  - `2026-07-18` 产品标签验证新增只读 precision-corpus readiness 审计，分别输出材料性唯一映射候选、公司/报告期、完整年报/半年报 manifest 绑定、六行业文档覆盖及短缺 blocker；未匹配/歧义标签审计增加材料性行数、材料性公司数和最大收入占比，只用于人工复核排序。实测生产库候选/正式 manifest 均为 0；30 家隔离库排除歧义映射和跨来源明显重复后为 `51/381` 条、5 家公司、51 个公司/报告期，候选层缺有色与建材且六行业 manifest-bound 均为 0。下一阶段先归档高材料性标签对应正式报告、受控升级精确别名并重放隔离候选，再补足六行业文档覆盖和全局 381 条正式报告绑定样本；不设置每行业统一最低样本数，不得直接开启生产同步。
