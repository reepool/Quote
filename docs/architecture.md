...
Quote System v2.4.0 是一个现代化的金融数据下载和管理系统，采用异步架构设计，支持多数据源、多品种类型（股票、指数、ETF）、智能限流、断点续传、自动调度和实时监控。系统基于 Python 3.10+ 构建，使用 FastAPI、SQLAlchemy、APScheduler 等现代化技术栈。v2.4.0 引入了动态复权计算引擎，所有数据以非复权原始价格存储，复权在查询时实时计算。

### 🚀 v2.4.2 架构更新亮点
- **🧮 动态复权计算引擎** - 存储原始数据，查询时前复权/后复权实时计算
- **🌐 港股与全球市场拓展** - 接入港股日线下载，支持双源降级容灾，完善长效“幽灵股自动筛除”机制
...
- **AkShare**: 综合性金融数据接口（A股交易日历、实盘行情。新增对港股全面历史日线、实盘的获取，以及港股复权因子的推导）
- **Tushare**: 专业级金融数据服务
- **YFinance**: 雅虎财经数据接口（美股兜底，新增对港股`.HK`格式智能映射备用）
...
本架构文档反映了 Quote System v2.4.0 的最新架构设计。
