...
Quote System v2.4.x 是一个现代化的金融数据下载和管理系统，采用异步架构设计，支持多数据源、多品种类型（股票、指数、ETF）、智能限流、断点续传、自动调度和实时监控。系统基于 Python 3.10+ 构建，使用 FastAPI、SQLAlchemy、APScheduler 等现代化技术栈。当前版本采用动态复权计算引擎，所有数据以非复权原始价格存储，复权在查询时实时计算。

### 🚀 v2.4.3 架构更新亮点
- **🧮 动态复权计算引擎** - 存储原始数据，查询时前复权/后复权实时计算
- **🌐 港股与全球市场拓展** - 港股日线主链路为 `AkShare -> YFinance`，港股复权因子主链路为 `AkShare(qfq-factor) -> YFinance`
- **🗓️ 周维护收口** - 港股复权因子默认放入周末 `weekly_data_maintenance`，日更保持关闭
...
BaseDataSource (抽象基类)
    ...
    ├── AkShareSource (A股/港股历史与实时数据)
    ├── YFinanceSource (美股/港股历史数据兜底)
...
**核心模型**:
- `Instrument`: 交易品种信息
- `DailyQuote`: 日线行情数据（存储非复权原始价格）
- `AdjustmentFactor`: 复权因子事件（除权除息日、累积因子）
...
5. 智能限流控制
    ↓
6. 数据源获取非复权原始数据 (adjustflag=3)
    ↓
7. 数据验证和质量评估
    ↓
8. 非复权数据入库 (adjustment_type='none')
    ↓
9. 同步复权因子 (仅股票, 通过 get_adjustment_factors)
    ↓
10. 缓存更新
    ↓
11. 进度报告和通知
...
```
1. API 收到查询请求 (adjust=qfq/hfq/none)
    ↓
2. 从 DB 查询非复权原始行情
    ↓
3. 判断是否需要复权 (仅 stock 类型 + qfq/hfq)
    ↓
4. 加载缓存复权因子 (get_cached_adjustment_factors)
    ↓
5. AdjustmentEngine 动态计算复权价格
    ↓
6. 返回复权后的数据 (factor + adjustment_type)
```
...
本架构文档反映了 Quote System v2.4.3 的最新架构设计。
