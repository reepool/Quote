...
> **A 股指数治理**：指数不是股票意义上的上市/退市对象。普通当前日更会先运行 OpenSpec `add-index-master-governance` 对应的指数主数据治理，按官方指数生命周期过滤终止计算发布、停发或配置写入的长期无官方行情指数；CNIndex 官方列表中没有 `深交所行情代码`、只有 `.CNI`/CNI 主数据编号的指数会作为 metadata-only 身份保留，本地遗留的错误行情型 key 会被停用并从日更 `tradable_only` universe 排除。历史下载仍保留既有指数行情，不删除历史行，也不对终止日后的缺口做 forward-fill。官方指数行情源会作为支持指数族的优先来源，BaoStock/AkShare 继续作为 active 指数的 fallback。
