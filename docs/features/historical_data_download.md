...
> **A 股主数据同步**：普通 `daily_data_update` 会在抓取行情前刷新 `SSE/SZSE/BSE` 股票主数据；显式历史回补默认跳过当前主数据同步，但会使用当前数据库中的 active 股票池。因此新股因股票字典滞后漏掉的日 K，应先完成一次主数据同步，再执行 `/backfill <开始日期> <结束日期> <交易所...>`；Telegram 时间段模式会走区间补数路径，每个交易所只读取一次 active 股票池、每个品种只请求一次连续日期范围，并默认跳过 TDX Phase 2.5 审计。

> **A 股指数治理**：指数不是股票意义上的上市/退市对象。普通当前日更会先运行 OpenSpec `add-index-master-governance` 对应的指数主数据治理，按官方指数生命周期过滤终止计算发布、停发或配置写入的长期无官方行情指数；CNIndex 官方列表中没有 `深交所行情代码`、只有 `.CNI`/CNI 主数据编号的指数会作为 metadata-only 身份保留，本地遗留的错误行情型 key 会被停用并从日更 `tradable_only` universe 排除。历史下载仍保留既有指数行情，不删除历史行，也不对终止日后的缺口做 forward-fill。官方指数行情源会作为支持指数族的优先来源，`SSE index` 当前优先 `CSIndex`，`SZSE index` 当前优先 `CNIndex`；官方源返回非空但未覆盖请求区间内最后一个交易日时继续走 BaoStock/AkShare fallback。
