...
BSPI 复用同一个主数据、来源周期、持久化和报告流水线，但其语义是周频市场价格，因此使用独立的公开协会价格治理 adapter，不能标成 `industrial_indicator`。`cn_coal_bspi` 已加入既有 `special_commodity_domestic_spot_price_sync` 聚合任务，不新增 BSPI 专项任务；10日滚动窗口覆盖周报发布日期相对报告期末的延迟，应用重启后仍须完成首次聚合任务验收。
