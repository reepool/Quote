...
| 环渤海动力煤港口价 | 已区分两类不同口径：BSPI 是周度综合指数；`CCTD 环渤海动力煤现货参考价`是 5500K/5000K/4500K 的日度港口现货参考价，不得混为一条序列。CCTD `catid=698` 历史目录可枚举 2023-05-30 恢复发布后的计算基础记录，但正文存在明确的未经许可使用限制且公开目录连续性未获证明，因此日频 CCTD 继续阻断。秦皇岛煤炭网是 BSPI 编译源公开目录，可按 `info_dteStr` 发现周评；CCTDA 行业资讯公开目录是转载对照，逐篇来源探测曾发现48篇候选，45篇可由正文明确的报告周期和正文/标题唯一价格形成治理观测，连续可治理起点为报告期末2025-06-03。 | 研究主口径已切到独立的 `cqcoal_bspi_weekly_port_price` provider、`compiler_public_price` 主数据治理、`CMD.CN.COAL.PORT_PRICE.BSPI.CQCOAL.WEEKLY` 和 `cn_coal_bspi` scope。CCTDA 转载对照保留 `cctda_bspi_weekly_port_price`、`association_public_price`、`CMD.CN.COAL.PORT_PRICE.BSPI.CCTDA.WEEKLY` 和 `cn_coal_bspi_cctda`。两条序列都只保存 BSPI 事实值、明确报告期、发布日期和来源 URL，不调用 `robots.txt` 禁止的 CCTDA `/api`，不保存或分发文章正文。单位固定为 `CNY/ton`、币种为 `CNY`、`data_kind=market_price`，观测日为报告期末；它们不是 CCTD 日度现货价、实际长协成交价或港口库存。2026-07-15 CCTDA 全历史 dry-run（run_id=166）成功：47篇范围内候选文章形成45条观测，2篇明确未披露指标值，0解析失败、0未解决日期，覆盖2025-06-03至2026-06-30。首次生产 write（run_id=167）新增45条，重复 write（run_id=168）为45条全部不变。2026-09-16 CQCOAL 全历史 dry-run（run_id=448）形成447条可解析观测，生产 write（run_id=449）新增447条，重复 write（run_id=450）全部不变，覆盖2016-11-08至2026-09-15。两条序列都由同一个国内现货价格聚合任务维护；对照源转载滞后只披露 `comparison_source_observation_stale`，不得单独把聚合任务打成失败。 |
...
BSPI 复用同一个主数据、来源周期、持久化和报告流水线，但其语义是周频市场价格，因此使用独立的公开价格治理 adapter，不能标成 `industrial_indicator`。研究主口径 `cn_coal_bspi` 使用 `compiler_public_price`；CCTDA 转载对照 `cn_coal_bspi_cctda` 仍使用 `association_public_price`。两者都加入既有 `special_commodity_domestic_spot_price_sync` 聚合任务，不新增 BSPI 专项任务；10日滚动窗口只约束目录发布日发现范围，新发布且解析成功的周报即使报告期末早于窗口也落库，缺口只按本地最新期末新鲜度告警，不按每个周二对源站扫缺。对照源转载滞后不得单独把聚合任务打成失败。
...
CCTDA 转载对照的历史回补使用独立 scope，不得再把 `cn_coal_bspi` 理解成 CCTDA：

- 短窗口 dry-run：`/run special_commodity_observation_backfill scope_id=cn_coal_bspi_cctda start=2026-06-17 end=2026-07-01 dry_run`。
- 全历史 dry-run：`/run special_commodity_observation_backfill scope_id=cn_coal_bspi_cctda start=2025-06-03 end=2026-07-15 dry_run`。

秦皇岛煤炭网研究主口径回补使用 `cn_coal_bspi`：

- 短窗口 dry-run：`/run special_commodity_observation_backfill scope_id=cn_coal_bspi start=2026-09-01 end=2026-09-16 dry_run`。
- 全历史 dry-run：`/run special_commodity_observation_backfill scope_id=cn_coal_bspi start=2016-11-08 end=2026-09-16 dry_run`。

生产 write 和重复 write 仅在全历史报告无 blocker、解析失败为零且来源覆盖缺口均有明确原因后执行；重复 write 必须全部为 `unchanged`。两条序列都由同一个国内现货与价格基准聚合任务维护，不新增专项任务。
...
| 秦皇岛煤炭网 BSPI 周评目录 | `http://www.cqcoal.com/`（`/mars-web/indexmark/listPage`、`/mars-web/indexmark/infoById`） | 当前周频 BSPI 研究主源；按公开目录 `info_dteStr` 发现周评，只提取价格、明确报告期、发布日期和来源 URL。不得把首页 `infonew` 快照日期当成发布日，不得保存或分发正文 |
| 中国煤炭运销协会 BSPI 公开文章目录 | `https://www.cctda.org.cn/list-6-1.html` | 周频 BSPI 转载对照源；按公开目录发现文章，只提取价格、明确报告期、发布日期和来源 URL。不得调用 robots 禁止的 `/api`，不得顺序扫描文章 ID或保存/分发正文 |
