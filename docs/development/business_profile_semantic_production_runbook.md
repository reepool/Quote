...
> **冻结通知（2026-09-02）**：旧公司画像 rollout、手工 backfill 和 semantic production
...
冻结前流程曾以自动化直通生产为默认路径。人工处理仅用于机器无法可靠消除的
...
冻结前配置曾进入 `structured_shadow`，以下启动说明当前已失效：
...
`acquire -> parse -> semantic -> verify -> publish` 有界推进队列并输出覆盖/队列对账。
