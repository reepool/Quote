...
10/25/50 provider-backed 高并发验证。延期不是通过：必须保留失败证据，并将 10 -> 25 -> 50
严格门禁留作供应商恢复后的容量认证和扩容前运维验收。部署负责人可以基于完整离线证据显式
接受风险并启用现有保守 route/pool；这不构成容量认证，且在门禁通过前不得据此提高并发、RPM
或声明 live rollout 通过。实现完成仍需通过离线 10/25/50 并发、重试、故障转移、熔断、
自适应降档/恢复和资源清理测试。
...
Provider-backed 10/25/50 高并发是容量认证与扩容门禁，不是供应商不稳定期间的实现完成门禁；
如明确延期，不得将未执行阶段标记为通过。部署负责人可显式接受风险启用现有保守 route/pool，
但不得在阶段通过前提高限额或声明容量认证完成。
...
12. 供应商恢复后按 10、25、50 并发逐级完成容量认证与扩容验收，并保留单源配置回滚路径；
    如因供应商不稳定延期，部署负责人可在完整离线验收后显式接受风险启用保守 route/pool，
    但不得提高限额或把延期标记为通过。
