...
> 状态：`initial_synthesis_complete`
> 日期：2026-09-02
> 样本版本：`manufacturing_materials.2026-09-02.1`
...
| `processing_volume` | recycling/processing narrative | coating/CAAS | outsourced processing | repair/service | `subtype_specific` | 加工服务与产品销量不能共用一个 metric |
...
- 表格主体优先读取明确表头、章节、合并/母公司说明和 named subsidiary；
...
- regime 变化使用 event/effective date；中航成飞样本的成飞股权过户生效日为 2025-01-06；当前包不得追溯覆盖此前期间。
...
- `processing_volume` 是否作为独立 metric type，而不是 `sales_volume` 的 subtype；初标建议独立。
- 产品/行业/地区表中“其他”“抵消项”的结构化类型和研究视图展示方式。
- subject 决策树对未显式写“合并口径”的管理层讨论表应采用何种最小证据。
- 中航成飞同一控制合并比较数如何与 predecessor 当时知识时点事实并列展示。

这些问题不阻止形成 v1 研究合同，但在外部审核和用户 acceptance 前保持 `in_review`。
