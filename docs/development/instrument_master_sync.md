...
- 交易安排/产品停运行写入顶层 `lifecycle_evidence_at`。当前不可交易窗口内，复牌或官方 active 行不得把 `trading_status` 清回 1；窗口过期后资格快照不再出行动，才允许恢复。
- 交易资格源：同一套接口再扫 `Trading Arrangements` / `Capital Reorganisation` / `Listing by Introduction` / `Privatisation/Withdrawal` / `CIS`。分类仍看 headline category，表格词只看标题、不看 SHORT/LONG 栏目名。撤回上市只认已经停止买卖的标题（last day of dealings / cessation of dealing / 已决定 cancellation of listing），拟私有化、月度更新、预案失效、NEEQ 摘牌不写。CIS 只落到基金/ETF 代码；只终止 USD/RMB 柜台时不写剩余 HKD 柜台，且柜台终止必须带已到期的生效日。HKEXnews 一条公告常把正股、票据和份额写在同一个 `STOCK_CODE` 里：撤回上市若夹带票据/窝轮或标题写 notes，不写正股。时间窗从标题里的生效日/复牌日读取：并股已生效且没有未来复牌日的不写；介绍上市空窗、ETF 停买/终止才写 `status=active,trading_status=0`，source 为 `hkexnews_trading_arrangement` 或 `hkexnews_product_cessation`。带明确复牌/开盘日的交易安排到期后不再出资格行，safe-write 会把 `trading_status` 写回 1。没有复牌日的产品停运（ETF/CIS 终止、已确认撤回上市）会保留 `source=hkexnews_product_cessation` 和 `trading_status=0`，不因公告滚出 8 个月回看窗口而恢复。港股日更 yfinance 备源不接受 expected day `volume` 为空或 0 的平移 K 线，且不把这种拒绝计入 stale circuit breaker。
