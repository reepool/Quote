...
- 短暂停牌源：同一套 `AnnouncementAcquisitionService` + `hkexnews` provider 扫描 HKEXnews 标题分类 `Trading Halt` / `Suspension` / `Resumption`。分类看 headline category / `[Trading Halt]` 标签，不看自由文本标题。按 `published_at` 取每只股票最新事件：停牌/停牌中写入 `hkexnews_trading_halt`，复牌不写停牌行。本地仍是 `status=suspended` 且官方 active 列表仍在、停牌源可用时，`mark_instrument_active` 会恢复 `status=active,trading_status=1`。空的停牌快照仍视为源不可用，不会据此批量复活。AkShare / 东财只做补充诊断，不是生命周期权威。
- 交易资格源：同一套接口再扫 `Trading Arrangements` / `Capital Reorganisation` / `Listing by Introduction` / `Privatisation/Withdrawal` / `CIS`。分类仍看 headline category；CIS 和撤回上市只用官方表格标题词（cessation / termination / withdrawal of listing 等）排除月报。时间窗从标题里的生效日/复牌日读取：并股已生效且没有未来复牌日的不写；介绍上市空窗、ETF 停买/终止才写 `status=active,trading_status=0`，source 为 `hkexnews_trading_arrangement` 或 `hkexnews_product_cessation`。到期日过后不再出资格行，safe-write 会把 `trading_status` 写回 1。港股日更 yfinance 备源不接受 expected day `volume` 为空或 0 的平移 K 线，且不把这种拒绝计入 stale circuit breaker。
