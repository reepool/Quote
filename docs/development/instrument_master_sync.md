...
- 交易安排/产品停运行写入顶层 `lifecycle_evidence_at`。当前不可交易窗口内，证券主列表、HKEXnews active 或更早的复牌证据不得把 `trading_status` 清回 1。人工复核 `action=active` 优先于产品停运；后续正式复牌或已到期的 `expected_resume_date` 可以结束停运。窗口过期后资格快照不再出行动，才允许 listing 行恢复。
