...
主数据新鲜度现在由 `DataManager.ensure_instrument_master_fresh()` 统一治理。该入口包装现有 `sync_instrument_master()`，只负责判断是否需要同步、是否复用本地新鲜状态、历史任务是否跳过、失败后是否继续，以及把结果写入报告；A 股股票主数据的实际源优先级和 upsert 规则仍只在 `sync_instrument_master()` 与 `DataSourceFactory.get_instrument_list()` 中维护。
...
历史缺口修复和区间回补现在使用 `data_config.repair_universe_governance` 做本地主数据生命周期过滤。该过滤不是上游主数据强制刷新：默认只读取本地 `instruments`、指数生命周期证据和本地最新行情日期，在 `detect_data_gaps()`、`fill_data_gaps()`、`find_gap_and_repair`、月度完整性检查和 `daily_data_backfill_range` 发起行情源请求前排除或裁剪无效窗口。典型跳过包括 `calculation_terminated/inactive/stale_no_quote/metadata_only` 指数、请求区间完全晚于 `delisted_date` 的品种、以及本地指数行情长期陈旧且命中 stale-no-quote 策略的停编候选。A 股指数优先使用 `index_lifecycle_evidence.last_quote_date` 作为终端行情边界；若缺少最终行情日但有官方 `effective_date`，可用 `index_lifecycle_effective_date` 作为较弱边界；只有缺少可用生命周期边界时，才允许 `index_lifecycle_stale_no_quote_fallback` 或 `index_delisted_after_last_quote_fallback` 这类降级兜底。报告会额外给出 `clip_reason_distribution`、`degraded_fallback_count` 和 warning，避免把启发式兜底误读为正式主数据证据。HKEX `status=suspended,trading_status=0` 的长期停牌股会按本地最新行情日裁剪历史窗口；若请求区间完全晚于最新行情日，则以 `hkex_suspended_after_last_quote` 计入生命周期跳过，不再把停牌后的无交易状态计为可修复缺口。HKEX active/tradable 但缺少 `listed_date` 的当前标的，会用本地首个行情日作为保守检测起点并标记 `hkex_missing_listed_date_local_first_quote`，避免把上市前或入库前窗口算成缺口；若同时没有任何本地行情，则以 `hkex_missing_listed_date_no_local_quote` 跳过并等待日更/主数据补齐上市边界。该规则不改写主数据上市日，后续仍应优先补充官方上市日期。报告会把这些计入 `repair_universe`，与 source empty、timeout、失败跳表分开。本地完全没有行情记录的 active 指数不会被直接判定为停编；`find_gap_and_repair` 会先允许有限探测，指数连续无数据失败超过 `max_index_no_data_failures_per_instrument`、其他标的超过 `max_no_data_failures_per_instrument` 后，只在本次任务内跳过同标的后续 gap，并写入 `gap_skip_list` 做 TTL 降噪。

历史任务默认仍保持 `skip_for_backfill` 语义，不会因为启用 repair universe 过滤而刷新 `exchange_official`、BaoStock、CNIndex 或 CSIndex 当前主数据。周度 `find_gap_and_repair` 例外：由于 HKEX 停牌/复牌状态直接决定是否应检测行情缺口，该任务会在检测前仅对 `hkex_instrument` scope 执行 HKEX lifecycle 前置治理，不触发 A 股全市场刷新。如果运维人员要在其他历史修复场景刷新当前主数据，必须显式传入 `force_current_master_refresh=true`，并可用 `current_master_refresh_scopes` 限定到如 `a_share_index` 或 `hkex_instrument` 的单一治理 scope；报告会标记这是 operator-requested historical repair refresh。绕过生命周期过滤只允许 `override_lifecycle_filter=true` 且带明确 `instrument_ids` 或小范围 `repair_universe_limit`，用于取证式修复，避免再次触发全市场无界源请求。
...
- 行情日更命中 `force_refresh_job_names`，默认每次当前日更都跳过 freshness 复用并按 `exchange_official -> BaoStock -> AkShare` 拉取 A 股股票主数据。
...
- 历史回补默认跳过当前主数据同步，避免用今天的股票状态改写历史回补语义；报告中会记录 `historical_backfill_current_master_sync_skipped`。`daily_data_update` 的午夜后前一日重试例外：例如 2026-06-16 00:18 手工重跑 `/run daily_data_update 2026-06-15`，仍按当前日更重试处理并强制运行 A 股股票和指数主数据治理；更早日期才按历史回补跳过。

A 股主数据没有独立自动 cron 任务；自动触发主要来自 `daily_data_update` 的前置治理和已接入治理的研究/财务/当前快照任务。`config/05_scheduler.json` 中 `daily_data_update` 为周一至周五 `20:00` 运行，当前默认强制刷新 A 股主数据；若后续从 `force_refresh_job_names` 移除该任务，则是否真实刷新会回到 `freshness_threshold_hours` 控制。需要单独验证股票主数据时，使用 manual-only 任务 `/run a_share_stock_master_sync`；它只运行 `a_share_stock` 治理，不下载日更行情，也不触发指数治理。HKEX 主数据有独立 `hkex_instrument_master_sync` 任务，但当前配置同样为 `manual_only=true`。
...
1. 当次 `daily_data_update` 是否实际执行了 `a_share_index` 前置治理，而不是被误判为普通历史回补。
2. `daily_data_update.parameters.instrument_types` 是否包含 `index`。
3. 这些代码在 `instruments` 中是否仍为 `active`；治理成功后应转为 `metadata_only`，并从 `tradable_only=True` 的日更 universe 排除。
...
- 普通指数日更要求行情源覆盖请求区间内最后一个交易日；官方源返回非空但最新行情日仍早于该交易日时，视为 stale 结果并继续 fallback。覆盖判断使用交易日历，不直接使用原始 `end_date`。同一进程内同一官方源、交易所、品种类型和目标交易日连续 stale 达到 `routing.daily_behavior.*.index.stale_source_circuit_breaker_threshold` 后，会临时熔断该源并在本轮后续请求直接走 fallback，避免 CNIndex/CSIndex 批量晚更新时重复空耗。熔断打开后每隔 `stale_source_circuit_breaker_probe_every` 个跳过品种探测一次官方源；探测已覆盖目标交易日则关闭熔断。HTTP 403/429 与源不可用（例如 pytdx 连接健康探针失败）共用 `transport_error_circuit_breaker_threshold`，达到阈值后同样跳过该源，不在日更主循环 sleep。全市场循环结束后，`data_config.daily_update_unresolved_retry` 会对仍为 `empty_unresolved` 的代码按配置源再请求一次（默认 `sources=["primary"]`，忽略本轮熔断）；源此时已恢复 T-day 则可挽回覆盖，否则保持 warning。
...
- `exchange_official` 是 A 股股票主数据主源，按交易所解析上交所、深交所、北交所的官方当前股票列表。配置入口是 `data_sources_config.exchange_official`，路由入口是 `routing.instrument_list.a_stock = ["exchange_official", "baostock", "akshare"]`。
- `BaoStock` 是第一备源和交叉校验源。官方源不可用或不可解析时，BaoStock 可以接管当前列表；官方源已成功时，BaoStock 只允许补充非生命周期字段和提供差异诊断，不能把官方 current list 缺失的代码并集补入 active universe。
- `AkShare` 是第二备源和候选发现源。官方源和 BaoStock 都不可用时，AkShare 可以接管当前列表；接管结果在报告中标为 `source_authority=akshare_fallback`，不具备退市日期和生命周期终局权威。
...
官方源主导后，报告会展示 `source_authority`：

- `official`：官方源提供完整 current list。
- `official_with_fallback_fields`：官方源确认生命周期，BaoStock/AkShare 仅补充行业、板块、上市日等非终局字段。
- `baostock_fallback`：官方源失败或不可用，BaoStock 接管。
- `akshare_fallback`：官方源和 BaoStock 均失败，AkShare 接管。
- `degraded`：所有源不可用或返回不可采信数据。

官方 current list 缺失不能单独写退市；只有官方终止上市、摘牌、退市公告或等价终局证据才允许写 `status=delisted,is_active=0,trading_status=0,delisted_date`。当前列表缺失但无终局证据的标的会进入 `instrument_master_discrepancies`，作为待复核候选。
...
现有 `DataSourceFactory.get_instrument_list(force_refresh=True)` 会按 `instrument_list.a_stock = ["exchange_official", "baostock", "akshare"]` 路由获取数据。官方源正常返回时，BaoStock/AkShare 不再把官方列表缺失的 instrument 并集补入 active universe，只补充非生命周期字段并记录 fallback-only discrepancy；官方源失败或返回不可采信列表时，才按 BaoStock、AkShare 顺序接管，并在报告中降级 source authority。
...
- `summary` 展示新增、停用、活跃合计和 source authority 分布。
...
- suspension 源：HKEXnews prolonged suspension monthly PDF（Main Board / GEM）。抽字走共享 PDF profile（默认 `pdfium_native`，可用 `pdf_profile` 或 `QUOTE_PDF_ENGINE_PROFILE` 回滚到 `pypdf_native`）。行解析按 `Link to HKEXnews` 切块，再用括号代码和日期确认公司，兼容 pypdf / pdfium 的空白差异。主板和创业板分开记账：只有成功且非空的一侧会叠加名单。解析 0 行或抓取失败时视为该市场停牌源不可用：报告 warning，且不会用失败一侧去清本地长停牌。`source_usage` 按 `hkexnews_suspension_report:Main Board` / `:GEM` 分开计数；Telegram「可复活」只统计过完跳过门后会写库的名字。`mark_instrument_suspended` 会把主板/创业板写入 `instruments.market`。safe_write 插入官方停牌行时直接写 `status=suspended,trading_status=0`。治理 warning/error 时港股日更仍拉行情，沿用库内生命周期，Telegram 会标明这一点。上市/退市同时出现的约 250 条冲突继续只进 review，不自动写生命周期。
- 短暂停牌源：同一套 `AnnouncementAcquisitionService` + `hkexnews` provider 扫描 HKEXnews 标题分类 `Trading Halt` / `Suspension` / `Resumption`。分类看 headline category / `[Trading Halt]` / `[Resumption]` 标签，不看自由文本标题。按 `published_at` 取每只股票最新事件：停牌/停牌中写入 `hkexnews_trading_halt`，真实复牌写入 `hkexnews_trading_resumption`（`status=active,trading_status=1`）。标题写明未来 resume/commence 日期时，复牌日未到保持 `suspended/trading_status=0` 并保留复牌证据。标题仍是 continued/still suspended、没有 `RESUMPTION OF TRADING` / 履行复牌指引等复牌动作词的，即使被扫进 Resumption 分类也按停牌处理。本地仍是 `status=suspended` 且官方最新证据已是 active 时，`mark_instrument_active` 会恢复 `status=active,trading_status=1`。空的停牌快照仍视为源不可用，不会据此批量复活。本地 source 仍是 `hkexnews_suspension_report` 的长停牌，只有该名字所属市场的月报本轮成功且非空，才允许仅因“不在该侧名单”而复活；市场未知时要两侧都成功。若同一次扫描拿到更晚的复牌公告，复牌日期覆盖月报 `as at` / `Posted on` 日期。短暂停牌快照在场但对应市场 PDF 失败时，不得只靠上市名单把这些 PDF 名字写回可交易。公告扫描失败或不完整时，safe-write 不得覆盖已有 `trading_status=0`。AkShare / 东财只做补充诊断，不是生命周期权威。
- HKEXnews 标题搜索按官方 Load More 协议分页：同一窗口用更大的 `rowRange` 继续取，并消费 `hasNextPage` / `hasNextRow` / `recordCnt`。`max_pages` 用尽仍有下一页时 `is_complete=false`，不得把残缺扫描当成“没有事件”。
- 退市写入只依赖 delisted 证据门控，不再要求本轮 official active 快照非空。官方退市记录优先于停复牌扫描残留的 active 行；若同一代码同时出现在证券主列表/HKEXnews active 和退市列表，进入人工复核而不是自动退市或自动恢复。active / safe-write / reactivation 仍使用 active 源门控。
- 交易安排/产品停运行写入顶层 `lifecycle_evidence_at`。当前不可交易窗口内，证券主列表、HKEXnews active 或更早的复牌证据不得把 `trading_status` 清回 1。人工复核 `action=active` 优先于产品停运；后续正式复牌或已到期的 `expected_resume_date` 可以结束停运。窗口过期后资格快照不再出行动，才允许 listing 行恢复。`mark_instrument_untradable` 只按合并后的 official active 行写入，已被复牌、人工 `active` 或到期恢复覆盖的旧资格行不再回写。
- 交易资格源：同一套接口再扫 `Trading Arrangements` / `Capital Reorganisation` / `Listing by Introduction` / `Privatisation/Withdrawal` / `CIS`。分类仍看 headline category，表格词只看标题、不看 SHORT/LONG 栏目名。撤回上市只认已经停止买卖的标题（last day of dealings / cessation of dealing / 已决定 cancellation of listing），拟私有化、月度更新、预案失效、NEEQ 摘牌不写。CIS 只落到基金/ETF 代码；只终止 USD/RMB 柜台时不写剩余 HKD 柜台，且柜台终止必须带已到期的生效日。HKEXnews 一条公告常把正股、票据和份额写在同一个 `STOCK_CODE` 里：撤回上市若夹带票据/窝轮或标题写 notes，不写正股。时间窗从标题里的生效日/复牌日读取：并股已生效且没有未来复牌日的不写；介绍上市空窗、ETF 停买/终止才写 `status=active,trading_status=0`，source 为 `hkexnews_trading_arrangement` 或 `hkexnews_product_cessation`。带明确复牌/开盘日的交易安排到期后不再出资格行，safe-write 会把 `trading_status` 写回 1。没有复牌日的产品停运（ETF/CIS 终止、已确认撤回上市）会保留 `source=hkexnews_product_cessation` 和 `trading_status=0`，不因公告滚出 8 个月回看窗口而恢复；人工 `active`、后续正式复牌或已到期复牌日可以结束该状态。港股日更 yfinance 备源不接受 expected day `volume` 为空或 0 的平移 K 线，且不把这种拒绝计入 stale circuit breaker。
