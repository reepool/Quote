...
- `2026-07-18` 新增产品标签 precision-corpus readiness 只读审计和材料性标签排序。生产库当前材料性唯一映射候选 `0`、合格正式报告 manifest `0`；30 家隔离回补库在排除一对多歧义映射和跨来源明显重复后为 `51/381` 条材料性唯一映射候选，覆盖 5 家公司和 51 个公司/报告期，候选层缺有色与建材，六行业 manifest-bound 均为 0。未解决标签审计增加材料性行数、公司数和最大收入占比，但不自动判断“其他/未分配”等标签含义。后续须先以正式报告证据扩充高材料性精确别名、重放隔离候选并补齐六行业文档覆盖及全局 381 条 manifest-bound 样本，再运行 99% 比较；不要求每行业统一达到固定样本数，也不能通过扩大煤炭样本或排除其他行业凑数。
