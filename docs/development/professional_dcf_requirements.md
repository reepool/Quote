...
| 优先级 | 字段 | 来源与用途 |
|---|---|---|
| P0 | 净利润、净资产、股本、净资本 | `net_income / equity / shares_outstanding` 来自现有本地财务与估值输入；`net_capital` 必须通过上市证券公司年报/半年报内嵌“净资本及风险控制指标”表进入财务披露链路，是证券 DCF production blocker |
...
| 证券公司主体 gate | CSRC 证券公司名录、证券经营机构信息公示、上市主体到证券公司牌照主体的显式映射 | 证券 | 申万“证券Ⅲ”只作为候选池；未通过 `listed_broker_dealer_scope` 的标的不得走证券公司 DCF |
| 证券监管指标 | 核心/附属净资本、风险覆盖率、资本杠杆率、LCR、NSFR、净资本/净资产、净资本/负债、自营和融资类业务占净资本比例 | 证券 | 年报/半年报内嵌风控表为主源；`net_capital` 为 blocker，其余为风险诊断和置信度输入 |
| 证券风险资本准备 | 市场/信用/操作/特定风险资本准备及合计、表内外资产总额、LCR/NSFR 分解项 | 证券 | P2 拆解字段，年报/半年报或独立风控报告明确披露才采集 |
| 证券业务分部收入 | 经纪、投行、资管、自营收入 | 证券 | 应来自年报分部/附注；不得由监管风控表的操作风险收入行替代 |
...
- `2026-07-18` 结构化业务画像已补齐统一 bounded sync 和治理 CLI：按申万时点行业及上市生命周期精确选择 A 股公司，支持来源/行业/公司 scope、数量/时长硬上限和公司×来源 checkpoint 续跑；候选写入使用内容寻址 gzip raw cache/manifest，运行报告汇总来源健康、报告期、payload 变化、别名命中和 DCF 零泄漏。30 家分层来源 pilot 与隔离临时库回补已完成，但生产配置和 scheduler 仍关闭，须先通过正式报告精度、目录 promotion 和单行业 production candidate pilot。
- `2026-07-18` 六行业各 5 家分层结构化来源 pilot 已完成：覆盖 BSE 4、SSE 8、SZSE 18，东方财富和同花顺均 30/30 成功，总耗时约 32 秒且 DCF 泄漏为 0；但东方财富 24/30 达到 200 行疑似上限，2,797 条产品行仅 61 条精确命中（覆盖率 2.18%），并存在少量非标准报告日期。该结果只证明免费来源传输链可用，不满足公司级商品暴露或 DCF production promotion。
- `2026-07-18` 同一 30 家样本已在隔离 `/tmp` research DB 完成 candidate backfill、失败来源 checkpoint 恢复和全量重放：最终 60 条 candidate evidence、5,554 条 candidate segment，全量重放新增 0，approved、价值链角色和公司商品暴露均为 0，`quick_check=ok`。这证明候选链幂等和 DCF 隔离成立，但不替代产品标签 99% precision gate。
- `2026-07-18` 业务画像审核闭环已实现：候选记录通过显式 operator switch 执行 approve/reject/supersede，使用预期状态和更新时间做并发保护；非 evidence 事实必须引用同公司 approved evidence，非官方 evidence 还必须留下复核依据。状态更新与不可变审核审计同事务提交，结构化 upsert 通过 SQLite conflict 条件防止并发降级终态；queue/audit 命令不初始化或修改数据库。正式报告标签复核不用程序推断 PDF 语义，但必须绑定 `financials.db` 中同公司、同报告期、官方来源层、有效且未被替代的完整年报/半年报 manifest，并校验本地归档 SHA-256；人工结论指定文档 hash 和页码，排除原因受限且默认排除率不得超过 5%。双侧 95% Wilson 下界要求至少 99%，即零错误也至少需要 381 条有效样本；只接受唯一产品映射，并去除同公司、同报告期、同规范标签和同产品映射的跨来源明显重复，不按公司或行业一刀切去重。先前 `/tmp` 六份 PDF 目录产生的 1 条候选没有 manifest 绑定，只保留为工具诊断；当前后述 51 条材料性候选均已绑定生产合格 manifest，但样本总量和行业覆盖仍不足，production promotion 继续阻塞。
- `2026-07-18` 结构化业务画像已接入唯一的 `business_profile_structured_sync` 周度有界 scheduler job，沿用 `ScheduledTasks -> DataManager -> StructuredBusinessProfileSyncService`，没有新增来源专用或行业专用任务。scheduler job、业务画像模块和免费结构化源均保持默认关闭；即使误触发任务，模块关闭也会在 provider 构造前短路。未来启用仍受候选写 operator switch、配置数量/时长上限、checkpoint/resume 和 DCF 零泄漏门槛约束，不调用延后的 LLM 接口；`resume=true` 在 checkpoint 不存在时创建首批状态，只有已有且 scope 一致时才恢复。调度与服务测试覆盖单源失败降级、payload 未变化、首次启动、中断、恢复、disabled 和非到期 scope；该接线不代表 `6.3` 正式报告精度或 `6.5` 单行业生产 pilot 已通过。
- `2026-07-18` 新增产品标签 precision-corpus readiness 只读审计和材料性标签排序。生产库材料性唯一映射候选仍为 `0`；30 家隔离回补库在排除一对多歧义映射和跨来源明显重复后为 `51/381` 条材料性唯一映射候选，覆盖 5 家公司和 51 个公司/报告期，候选层缺有色与建材。未解决标签审计增加材料性行数、公司数和最大收入占比，但不自动判断“其他/未分配”等标签含义。后续须先以正式报告证据扩充高材料性精确别名、重放隔离候选并补齐六行业文档覆盖及全局 381 条 manifest-bound 样本，再运行 99% 比较；不要求每行业统一达到固定样本数，也不能通过扩大煤炭样本或排除其他行业凑数。
- `2026-07-18` 新增材料性 precision 候选驱动的官方报告发现/归档编排入口，复用 CNInfo 主源、交易所备源、不可变文件、manifest 与 checkpoint；默认只读，归档写入要求字面量 `BUSINESS_PROFILE_OFFICIAL_ARCHIVE_WRITE`，并报告候选表前后差值。`000717.SZ` 2025 年完整年报已完成首个生产归档试点，文件命名、SHA-256、manifest、父子 ingestion lineage 和断点清理均通过，未写任何画像候选或 DCF 输入。限定 2025 年的隔离候选为 2 行、manifest-bound 为 1 行，99% precision 所需 381 条样本仍未满足。
- `2026-07-18` 首批 5 家公司的 51 个候选报告期已全部完成官方完整报告发现、不可变归档和 manifest 绑定，SHA-256 校验错误、缺失期间、失败文件和残留 checkpoint 均为 0，候选表差值仍为 0。发现链修复了巨潮实际 30 行分页上限、宽关键词窗口挤出历史报告、主源部分缺期不调用交易所备源，以及“年度半年度报告/半年报”官方标题未识别问题。当前 readiness 为 `51/381`，还差 330 条且缺有色和建材材料性唯一映射候选，因此只完成报告证据基线，未通过 99% precision gate。
- `2026-07-19` 官方报告编排补充显式 `catalog_issues` 目标范围，允许材料性未匹配/歧义产品标签先驱动有界正式报告取证，避免目录 promotion 因“必须先精确映射才能归档报告”而形成闭环阻塞。默认仍为 `precision_exact`；新范围只读取 `alias_not_found / ambiguous_product_alias` 候选，只保留合法 `06-30 / 12-31` 年报/半年报期间，同一事实跨来源合并诊断并保留收入占比最高的代表行；不会自动扩充产品目录、批准业务事实、生成公司商品暴露或进入 DCF。30 家隔离库共识别 2,096 条问题标签、30 家公司和 486 个公司/报告期；建材 `000012.SZ` 与有色 `000506.SZ` 2025 年报的只读巨潮探测均成功且候选表差值为 0。`6.3` 仍须完成正式页码人工核对、受控精确别名 promotion、隔离重放和 381 条 manifest-bound 样本。
- `2026-07-19` 产品目录 promotion 的证据门槛已收紧：治理 CLI 不再接受任意 `evidence-reference` 字符串，必须提供 `business_profile_product_alias_official_evidence.v1` JSON 并连接 `financials.db`。写入新目录前强制验证同公司/报告期的活动官方完整年报或半年报 manifest、未被替代状态、归档文件 SHA-256、PDF 实际页数、引用页原生文本及 hash、正式标签在至少一个引用页出现、产品/行业结论、审核人、带时区审核时间和理由，并要求审核时间不晚于 promotion 时间；相对归档路径按显式基准目录解析，不再依赖进程工作目录。验证后的证据摘要及 hash 写入不可变 promotion manifest，但不复制本地归档路径或页全文。扫描件和文本解码异常页继续 fail closed，留待受控 OCR artifact 处理，不以语义推断替代原文核验。该能力关闭了伪造引用绕过正式报告复核的路径，不代表已有任何新别名通过人工审核。
- `2026-07-20` 新增 `business_profile_product_catalog_issue_review.v1` 人工复核工作包和 CLI：把材料性 `alias_not_found / ambiguous_product_alias` candidate、来源行身份、行业、收入占比与同公司/报告期活动官方完整报告合并为带 row/package hash 的只读待审包；人工逐行填写 `promote_alias / defer / exclude`，只有 promotion 决定可导出 `business_profile_product_alias_official_evidence.v1`。导出会验证候选快照 hash、正式标签与来源标签一致、报告 ID/hash/期间、正页码、产品、行业、审核人和带时区审核时间，并保留 review/package/source lineage；快照 hash 只使用可移植的正式报告身份字段并规范化文档顺序，不受本机归档路径或 URL 影响，promotion 端会重算快照 hash 并核对来源行 lineage。最终目录写入仍执行 PDF 原文、页级 hash、目录版本和发布时间门槛。该能力只打通人工操作链，不自动判断产品、不自动批准候选，也未完成 `6.3` 的 381 条正式比较。
- `2026-07-20` 特殊商品行情已从“全局附加诊断”改为 approved 公司暴露驱动：resolver 可识别 `futures_series` 和 `commodity_price_series`，DCF 只加载公司 mapping 指定的特殊商品序列，按估值日过滤观测并输出角色、映射、周期分位和 lineage。单一 approved 收入端序列可作为 `commodity_cycle_context`；原材料/能源成本单腿、多序列以及跨期货/特殊商品序列未实现价差解析时 fail closed，不允许套用收入端周期方向。本地已有 44 个特殊商品、47 条序列和 102,053 条观测，但 `company_commodity_exposures` approved、`futures_spread_definitions` 和 `futures_exposure_mappings` 均为 0；因此只是补齐行情接线和方向门槛，`6.8` 的明确成本证据、人工批准映射和价差定义仍未完成。
- `2026-07-18` structured sync 审查后进一步 fail closed：来源证券代码必须匹配 instrument，请求和 retry 受剩余 deadline 约束；raw manifest 成功是 candidate 写入及 checkpoint 完成的前置条件，run 异常仍记录终态；DCF 泄漏不再使用固定零值，而是实测 approved、价值链角色和公司商品暴露的运行前后数据库差值。
