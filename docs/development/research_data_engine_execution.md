...
- 专业 DCF 当前仍属于实时计算路径，不写入 `valuation_history`；本轮实现优先完成普通非金融 FCFF、模型选择/对比、假设 lineage 和本地 readiness。银行/证券/保险/地产/周期/公用事业/控股公司等 profile 当前以 guardrail/partial 状态 fail closed，不得静默降级套用通用 FCFF。xlsx 投行级底稿、显式 assumption refresh、本地 bounded run cache、金融行业实算模型仍是后续 OpenSpec 任务。
