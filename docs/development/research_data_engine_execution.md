...
- 当前 A 股研究、财务和最新快照类写入任务在解析 `instruments.is_active` 股票池前，统一经过 `DataManager.ensure_instrument_master_fresh()` 主数据治理入口；该入口复用行情系统已有 `sync_instrument_master()`，不另建研究/财务专用股票字典逻辑
- 主数据治理分为 freshness-gated 前置治理和 `force_refresh_job_names` 强制刷新两种策略；当前默认强制刷新 `daily_data_update` 与 `industry_standard_sync`，其他当前研究/财务任务通常只在主数据过期时触发实际刷新
- 港股研究模块进入开发前，必须先依赖 `add-hkex-instrument-master-sync`：HKEX 主数据与 A 股主数据同级，是未来港股公司画像、财务、披露、估值、风险、事件和行情研究模块共用的 universe 底座；下游统一调用 `DataManager.resolve_hkex_current_universe()`，并透传 `readiness='degraded'` 的治理降级原因
...
- 当前已补齐仓库级 `valuation` rollout validation runner：
  ...
  - `--skip-sync` 模式只做 readiness 快速读链路复核，会跳过 `valuation_history_rebuild`；不得用于 `write_policy=overwrite` 修复或任何需要重算估值历史的场景。正式 rollout 前仍必须用 bounded rebuild 检查 `data/valuation.db.valuation_inputs` 与 `valuation_history` 的实表结果
  - 当前已新增直接估值历史重建 runner：`python scripts/research_valuation_history_rebuild.py --exchanges SSE,SZSE,BSE --quote-limit-days 7 --window-mode trading_days --write-policy overwrite`。该脚本只调用 `DataManager.run_valuation_history_rebuild()`，不执行 readiness 聚合，适用于补跑 `valuation_input_sync` 后重算最近窗口、股本历史/财务可得日/估值参数修复后的受控 overwrite，以及规避 rollout validation readiness 阶段的高内存开销
...
- 证券监管指标接入财务披露链路：历史回补默认按正式年报/半年报公告扫描上市证券公司 universe，命令如下；日更增量由调度后置任务扫描最近公告窗口并写 `hot` tier，可选扫描独立《风险控制指标报告》作为补充。`net_capital` 是 broker DCF blocker；风险覆盖率、资本杠杆率、LCR、NSFR、净资本/净资产、净资本/负债、自营/融资占净资本比例和风险资本准备分项作为证券 L1.5 专项事实与 DCF 风险诊断输入。监管风控表中的操作风险收入行如被披露，只能保存为监管口径 P2 字段，不能替代年报分部收入。
...
- 证券 DCF 数据供给已接入财务事实链路：`use-annual-report-broker-risk-control-source` 以 CSRC 名录 + 显式上市券商 scope gate 确认 universe，并以正式年报/半年报内嵌风控表作为 `net_capital` 与核心监管指标主源；独立风控指标报告只作为补充/校验源。历史回补与日更后置任务均写现有 canonical numeric facts，DCF input bundle 只读本地事实，不在估值请求中远程抓 PDF。
...
- `/api/v1/research/company/{instrument_id}/valuation/dcf/readiness`
- `/api/v1/research/company/{instrument_id}/valuation/dcf/input-gaps`
- `/api/v1/research/valuation/dcf/model-profiles`
- `/api/v1/research/valuation/dcf/assumptions`
- `/api/v1/research/valuation/readiness`
...
- `valuation_history` 更新默认使用 `write_policy=missing_only`：日更、周更和过去 `12` 个季度窗口任务均先轻量推导候选估值日期，再按完整主键 `instrument_id + as_of_date + calc_method + calc_version + parameter_hash` 检查缺失；窗口已完整的标的直接跳过完整 PE/PB/PS 指标计算，存在缺口时只写缺失行；显式 `write_policy=overwrite` 用于股本历史、财务可得日、计算逻辑或参数修复后的受控覆写。`missing_only` 不比较 `valuation_inputs.updated_at`，因此补跑或修复 input 后必须显式 overwrite 受影响窗口，否则已存在的估值历史不会自动重算
- 估值历史直接重建 runner 为 `scripts/research_valuation_history_rebuild.py`，用于只执行 `valuation_history_rebuild` 而不聚合 readiness。典型修复命令：`python scripts/research_valuation_history_rebuild.py --exchanges SSE,SZSE,BSE --quote-limit-days 7 --window-mode trading_days --write-policy overwrite`。该命令可一次性重算三市最近 `7` 个交易日；若机器内存或数据库写入压力偏高，再按交易所拆分执行
...
- 专业 DCF 当前仍属于实时计算路径，不写入 `valuation_history`；本轮实现已覆盖普通非金融 FCFF、稳定非金融 FCFE、公用事业/REIT 轻量 DDM/分派模型、银行 residual income、证券 normalized residual income + excess capital、资源周期 mid-cycle FCFF、模型选择/对比、FCFE/FCFF adapter、假设 lineage、本地 readiness、显式 assumption refresh diagnostics、投行级 xlsx workbook artifact 和进程内 bounded run cache，并已完成参数/API 合约硬化：比较输出返回候选 result object，FCFE 输入不足时 fail closed，suppression/terminal/scenario/workbook 参数具备明确语义，assumption fallback/缺失进入结构化诊断。保险/地产/控股公司等 profile 当前以 guardrail/partial 状态 fail closed，不得静默降级套用通用 FCFF；真实主备外部数据源刷新 adapter、跨进程 saved-run audit 和剩余金融/特殊行业实算模型仍是后续扩展。
...
3. 存储层扩展
  ...
  - `2026-05-24` 财务任务化实现启动：新增 `financial_l1_full_import` 手工 `/run` 入口，直接复用 `run_full_import()`；新增 `FinancialDisclosureIncrementalSyncService`，通过通用官方公告 acquisition service（当前路由以 CNInfo 为主源）识别定期报告、更正、延期披露、停牌、退市风险和可能终止上市公告，候选维度为 `instrument_id + report_period`；新增 `financial_disclosure_event_state` 记录 pending recheck、`pending_delisting_risk`、accepted gap、blocking gap 和处理结果。增量/对账任务默认不自动启用，先通过 `/run` 做小范围验证。
  ...
  - 同日继续复核第三次 reconciliation：`ingestion_run_id=446` 为 `accepted=376 / changed=2 / blocking=122`。122 个 blockers 中，119 个为 BSE，且 `quotes.db.instruments` 里 BSE `listed_date` 全部为空；联网确认 `akshare.stock_info_bj_name_code()` 实际返回 `上市日期 / 所属行业 / 地区`，但 `data_sources/akshare_source.py` 过去把 BSE `listed_date` 硬编码为 `None`。已修复 BSE 主数据解析并执行 BSE 主数据刷新，当前 315 个 BSE 标的均有 `listed_date`，对原 119 个 BSE blockers 重新套生命周期规则后全部为 `pre_listing_period`。剩余 `002731.SZ / 2025-12-31, 2026-03-31` 与 `688121.SH / 2026-03-31` 在 `financials.db.announcement_audit` 中已有 `source=cninfo` 的“无法在法定期限内披露定期报告/停牌/可能被实施退市风险警示”公告，但标题未含具体报告期；维护层已新增受限复用规则，仅将公告日前 180 天内的近端报告期标记为披露异常 accepted/pending delisting，避免误放更早历史期。定向 dry-run 覆盖 `920178.BJ / 002731.SZ / 688121.SH` 的近端期次，结果 `status=success`、`accepted_gap_count=5`、`blocking_gap_count=0`。
...
- valuation history rebuild runner：通过 `python scripts/research_valuation_history_rebuild.py --exchanges SSE,SZSE,BSE --quote-limit-days 7 --window-mode trading_days --write-policy overwrite` 直接重算估值历史，不执行 readiness 聚合；用于补跑 `valuation_input_sync` 后修复已存在估值行、参数/股本/财务可得日口径修复后的受控 overwrite。默认日更 `/run valuation_history_rebuild` 仍走 `missing_only`，只适合补缺口
- valuation rollout validation runner：通过 `python scripts/research_valuation_rollout_validation.py --sync-inputs --exchanges SSE,SZSE` 将 `valuation_input_sync -> valuation_history_rebuild -> readiness` 串成可重复执行的仓库级复核命令；支持 `--window-mode trading_days|last_12_quarters` 与 `--write-policy missing_only|overwrite`。其中 `--skip-sync` 表示跳过 `valuation_history_rebuild` 只查 readiness，不能用于 overwrite 修复
...
- `2026-04-27` 增量更新策略与 BSE 覆盖评估：
  - 股东户数存在轻量变化检测路径；此前工程策略按业务要求统一收口为周更，但 `2026-05-23` 已新增 OpenSpec `add-shareholder-incremental-sync` 并落地通用 CNInfo 公告元数据扫描/过滤/水位模块，股东域已改造为“每日公告驱动增量检查 + 周期性/手工全量刷新”双模式：
    ...
    - 新增通用公告扫描能力通过 purpose/exchange 路由选择官方来源；股东增量当前使用 CNInfo 市场范围分页能力，业务层传入标题/分类选择器并维护独立水位，筛出相关公告后再对候选标的拉取 `holder_count / top10_holders / ownership_clues` 并做规范化 hash 比较；只有 hash 或覆盖范围变化才写入快照
    - `shareholder_incremental_sync` 默认每日 `06:30` 运行，写入 `announcement_scan_state / announcement_audit / shareholder_change_manifest` 以解释来源限定的水位、候选、hash 和 pending recheck；同一批公告的 pending 截止时间固定为第一次进入 pending 的 `first_pending_at + pending_recheck_days`，不会因每日扫描滚动延长。旧 CNInfo 专用公告状态/审计表仅在初始化时做备份、对账迁移并删除，不属于当前运行时 schema
