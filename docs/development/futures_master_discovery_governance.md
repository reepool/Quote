...
- 不在本阶段一次性完成 INE、CZCE 所有规则页解析；DCE、GFEX、SHFE 已按交易所专用 adapter 接入。
...
DCE/GFEX/SHFE 使用各自官方产品规格页面路径；INE/CZCE 必须逐所实现官方产品页或交易参数 adapter。SHFE adapter 已验证官方 Safeline challenge、meta-refresh、`/products/futures/.../<symbol>_f/` 叶子页发现，以及 `pageList` 合约对象字段解析。通用产品页 parser 只能作为共享工具：每个交易所仍要单独确认入口页、详情页 URL 规则、字段标签、反爬/WAF 处理、失败报告和 live dry-run 结果。缺少关键字段的候选会保持 `discovered_unverified/pending`，不会自动进入正式主数据；官方页面被 WAF 或挑战阻断时必须写入任务 warning，而不是静默回退。
