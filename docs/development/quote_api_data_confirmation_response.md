...
> 首次回执：2026-07-13（第一轮）｜2026-07-13（第二轮，A 股历史行情回补后）｜**本次更新：2026-07-13（第三轮，退市股历史行情回补 + `/stats` 性能复核）**

---

## 更新说明（第三轮）

贵方反馈"补足了退市股票的历史行情，`/stats` 的性能劣化也解决了"，我方对全库重新核实（非采信反馈原文，逐项重新查库+打活服务），结论如下：

- **B1 从"部分解决"更新为"✅ 基本解决"**：退市 STOCK 447 只中现有 **446 只**有完整行情历史，覆盖 1999–2026 年**全部**退市年份分组（第二轮发现的"288 只 1999–2024 年退市股零行情"缺口已清零）。抽样验证最早的 2002 年退市股（如 `000003.SZ`）历史深度回溯到 1991 年真实上市日，行情延续到退市前夕；并通过**真实运行服务**（非仅查库）验证可查询到 1995 年的历史行情。**唯一剩余缺口是 `03588.HK`（富途控股-W，港股，`delisted_date` 为空）**，不属于本次 A 股退市股回补范围，如实标注。
- **A3 性能修复在更大数据量下依然有效**：`daily_quotes` 进一步增至 **3073.8 万行**（较第二轮再 +3.5%），`/stats` 直接调用与**真实运行服务**实测均为 **~2 秒**，未随数据量增长而劣化。
- **A9 数值微调**：`pre_close` 缺失率 **0.0251%**（与第二轮的 0.0260% 基本持平），结论不变。
- A1（历史深度）、A2、A4–A8、B2–B7 结论不受本轮变化影响，维持第二轮回执结论。
...
| A3 🔴 | **已修复（性能修复经第三轮更大数据量复核仍有效）** | `/stats` 路由改为正确读取嵌套结构 + `fast_mode=True`；3073.8 万行规模下实测仍为 ~2s，未随数据量增长劣化 |
...
| A9 | **已用全量数据回答（第三轮更新数值）** | 全库 `pre_close` 缺失率 **0.0251%**（约 7730/30738208），集中在近期个别交易日，非除权日 |
| B1 | **✅ 基本解决（第三轮）** | 447 只退市 STOCK 中 **446 只**已有完整行情历史，1999–2026 年各退市年份分组全部覆盖；唯一剩余缺口 `03588.HK`（港股，非本次 A 股回补范围） |
...
**第三轮复核（贵方完成退市股回补后，数据量进一步增至 3073.8 万行）**：直接调用与**真实运行服务**（`curl http://127.0.0.1:8000/api/v1/stats`）实测均为 **~2 秒**（1.81s / 1.998s），未随数据量继续增长而劣化——`fast_mode=True` 跳过的两个 group-by 是 `daily_quotes` 全表扫描，其耗时与表行数增长成正比，但已跳过，故整体响应时间对表规模不敏感，符合预期。性能修复确认稳定有效。
...
**结论：已用全量数据回答，非小样本。（历经三轮，数值随每次回补更新，结论持续不变）**

```
                第一轮（648.8万行）  第二轮（2970.8万行）  第三轮（3073.8万行，本次核实）
缺失行数：       7,709 行            7,730 行             约 7,730 行
缺失率：         0.1188%             0.0260%              0.0251%
```

集中日期（Top 10，三轮一致）：`2026-06-15`(856)、`2026-06-30`(133)、`2026-07-05`(124)、`2026-07-07`(116)、`2026-07-01`(101) 等——**均为近期交易日**，不集中于除权除息日。三轮新增回补的历史数据（含本轮退市股）均未引入新的 `pre_close` 缺失，缺失持续只集中在近期个别交易日，非除权事件导致的系统性缺失。

---

## 2. B. SRS §26 外部 Backlog（源端可得性联合验证）

### B1 退市股历史行情大规模回补（第三轮核实：已解决）

**结论（第三轮）：✅ 基本解决。** 贵方完成退市股历史行情回补后，第二轮发现的"288 只 1999–2024 年退市股零行情"缺口已清零，我方重新核实确认。

复核方式：按"退市年份"对全部 447 只退市 STOCK 重新做行情覆盖率分组统计（与第二轮同一方法，直接对比）。

```
退市年份分组          第二轮：已有行情      第三轮：已有行情（本次核实）
delisted_date 为空     120/121             120/121（不变，唯一缺口见下）
1999–2024（合计）      0/288               288/288 ✅ 全部覆盖
2025                   30/30               30/30
2026                   8/8                 8/8
合计                   158/447             446/447
```

样本验证（覆盖第二轮标记为"零行情"的最早退市年份，抽样 2002 年退市）：
```
600813.SH（2002-09-16 退市）: 行情范围 1994-01-14 ~ 2002-04-26，共 2008 行
000003.SZ（2002-06-14 退市）: 行情范围 1991-07-03 ~ 2002-04-26，共 2423 行（回溯到 1991 年真实上市日）
000556.SZ（2002-05-29 退市）: 行情范围 1994-05-25 ~ 2002-04-26，共 1698 行
```
以上均回补到接近其真实上市日、延续至退市前夕（与退市日之间的小间隔通常是停牌确认期，属正常现象）。

**真实运行服务复核**（非仅查库，确认对外可用）：
```bash
curl "http://127.0.0.1:8000/api/v1/quotes/daily?instrument_id=000003.SZ&start_date=1995-01-01&end_date=1995-01-10&include_delisted=true"
→ 200，返回 6 行 1995 年真实历史行情（close=5.78, source=akshare）
```

**唯一剩余缺口**：`03588.HK`（富途控股-W），**这是港股，不是 A 股**，且 `delisted_date` 为空（退市状态本身未最终确认）。不属于本次"A 股历史退市股回补"范围，如实标注，不计入本次核实的完成度扣分项。

**结论**：B1 关联的 `DELISTED_HISTORY_COVERAGE_GAP` 偏差可视为基本解除，贵方现在可用退市 STOCK 池构造消除幸存者偏差的历史回测资产池。

---

| 编号 | 结论 | 说明 |
|---|---|---|
| B1 | **✅ 基本解决（第三轮）** | 详见上方独立小节 |
...
A2、A3、A6 已修复上线（OpenSpec change `fix-instrument-stats-coverage-defects`），均为新增枚举值/修正内部计算逻辑/新增响应字段，未改变任何现有成功查询路径的返回结果结构。**已用真实运行服务确认生效**（`curl http://127.0.0.1:8000/...` 实测，非仅查库或查代码）。

A3 追加的性能修复（`/stats` 从超时恢复到 ~2 秒）已经过第二、三轮共两次数据量增长（2970→3073 万行）复核，均确认稳定有效，**真实运行服务实测同样为 ~2 秒**。

A4（默认排序方向是否改为统一升序）**未修改**——涉及现有默认响应顺序变更，按贵方"不改变现有端点默认行为"的原则，需贵方明确需要后再单独评估执行，不在本次自动修复范围内。

**A1（历史深度）、B1（退市股回补）均已在第二、三轮解决**（贵方完成回补，我方逐轮复核确认，B1 唯一剩余缺口为 1 只港股、不属于 A 股回补范围）。B2-B7（公司行为明细采集、财务修订 vintage、ST 状态历史等）均涉及数据重新获取或较大架构改造，按约定推至下一步，不在本次修复范围。

至此，A 类 9 项中 **7 项已解决/已修复/既定行为确认**（A1/A2/A3/A5/A6/A7/A9 中带确定结论者）、A4/A8 为需贵方决策或已澄清 SRS 假设、B 类中 B1/B2/B3 已有明确结论，B4-B7 待专项 spike。
