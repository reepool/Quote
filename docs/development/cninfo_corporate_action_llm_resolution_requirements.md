...
```json
{
  ...
  "economic_terms": {
    ...
    "bonus_shares_per_share": null,
    "capitalization_shares_per_share": null,
    "rights_shares_per_share": null,
    "rights_price": null
  },
  "evidence": [
    {
...
effective_date_type:
  ...
  share_arrival_date | listing_date | resumption_date | consideration_payment_date | unknown
```

`resolved_candidate` 必须具备非空 `effective_date`、`date_basis` 和至少一条正文证据。
其他状态的 `effective_date` 默认必须为空，除非作为 `alternative_dates` 明确记录冲突候选。
...
`validated_candidates` 才进入人工审核队列，`manual_required`、冲突、OCR 不可用或下载失败
不能作为有效日期。
...
- dry-run 可以下载/解析到临时 artifact，但不得调用模型或写生产库，除非显式指定
  `scan_sources=true`；
