...
| `special_commodity_domestic_spot_price_sync` | 国内特殊商品现货价格与基准同步 | 周一至周五 22:30 | 同步100ppi国内现货、NBS动力煤旬价、秦皇岛煤炭网周频BSPI研究主口径和CCTDA转载对照 |
...
滚动窗口任务默认回看最近10个自然日，而来源窗口任务使用来源公开边界。周频 BSPI 的10日窗只约束目录发布日；新发布且解析成功的周报即使报告期末早于窗口也落库。研究主口径 `cn_coal_bspi` 指向秦皇岛煤炭网编译源，CCTDA 转载对照走独立 scope `cn_coal_bspi_cctda`；对照源转载滞后不得单独把聚合任务打成失败，缺口只按本地最新期末新鲜度告警，不对源站按周二扫缺。各 scope 隔离来源、频率、报告和日期治理。国内现货、官方旬价和产业指标不进入期货连续合约任务，因为来源观测日、单值价格或指数和缺口治理不同于交易所交易日、合约生命周期和 OHLCV 治理。原始 FRED 序列继续独立保存用于来源审计；其他100ppi品种需完成逐品种治理和历史验证后再加入国内现货任务。World Bank/FRED-IMF 月频和政策事件继续使用独立频率。08:00缓存预热保持在08:20，避免同分钟竞争。其他特殊商品任务继续保持手工或未启用状态：
...
港口价格按独立语义治理。CCTD 5500K/5000K/4500K 日频现货参考价因许可限制和公开历史连续性不足继续阻断；不能因港口库存或 BSPI 来源可用而放行。研究主口径现为秦皇岛煤炭网编译源：独立 `cqcoal_bspi_weekly_port_price` provider、`compiler_public_price` 治理、`CMD.CN.COAL.PORT_PRICE.BSPI.CQCOAL.WEEKLY` 和 `cn_coal_bspi` scope。CCTDA 公开文章目录保留独立转载对照序列 `CMD.CN.COAL.PORT_PRICE.BSPI.CCTDA.WEEKLY` 和 `cn_coal_bspi_cctda` scope，不再充当研究主源；历史 CCTDA 回补仍以 `association_public_price` 治理、`run_id=166/167/168` 的 45 条观测和聚合 `run_id=169` 为准。两条序列均为 `CNY/ton`、`data_kind=market_price`，只保存价格事实、明确周报周期、发布日期和 URL，不保存正文、不调用 CCTDA robots 禁止的结构化 API，也不替代 CCTD 日价或实际长协价。两条序列均由同一个 `special_commodity_domestic_spot_price_sync` 维护，对照源转载滞后不得单独把聚合任务打成失败。
