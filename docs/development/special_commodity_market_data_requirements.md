...
环渤海港口煤炭库存已复用同一工业指标契约实现 CCTDA TTCI 周报 adapter、主数据、来源周期/发布日期治理和 `cn_coal_bohai_port_inventory` scope。该序列仅保存中国煤炭运销协会周报披露的环渤海港口合计库存事实值，单位为 `10k_ton`，观测日为周报统计期末，发布日期单独保存；不得解释为港口吞吐量、沿海电厂库存或商品价格，也不得把 TTCI 指数点当作 `CNY/ton`。公开 TTCI 目录自2024-08-02起可枚举，但早期周报并未披露该库存指标，当前同口径指标起点为首次实际披露日2025-02-08。adapter 将“报告存在但未披露库存”计入来源覆盖诊断而不是解析故障；对来源原文中调入、调出和库存字段发生明显错位的页面，仅当统计句内存在唯一符合配置数量级的库存候选时进行字段对齐并保留 reconciliation 标记，否则继续告警。当前 scope 仅允许手工回补且调度保持禁用，完成全历史 dry-run、生产 write 和幂等日更验证后才能加入共享产业指标任务。港口价格仍按增强需求中的独立来源阻断状态管理，不能因库存来源已解决而一并放行。
