...
> 历史状态（2026-09-02）：本文件不再作为新公司画像语义开发的权威需求。
> 当前唯一权威总需求为 `company_profile_product_and_industry_semantic_requirements.md`。
> 本文件仅保留旧实现背景，后续不得据此恢复旧 writer、旧回补或扩大商品暴露发布。

> 以下正文均为冻结前旧方案描述，所列自动化策略、启用状态和命令不再具有现行效力。
>
> 冻结前生产策略更新（2026-08-03）：本需求由 OpenSpec change
> `automate-business-profile-semantic-production` 统一控制。下文保留的 candidate-only、
> 逐行人工审批和人工目录包描述属于早期试点记录；其中“自动化直通”等表述仅说明冻结前方案，不得覆盖新总需求。

## 0. 自动化优先的生产原则

生产单元是 `(instrument, knowledge_cutoff, field_family)`，不是“公司全部历史公告”。系统先
检查已批准覆盖、时效、文档 hash 和未解决缺口，默认只选择最新有效年度完整报告；只有较新
半年报能更新时效字段、更正稿替代原稿、或受治理的专项公告能补足业务变化、重大合同、产能、
资源和套保字段时，才增加有限文档。计划完整后跳过更早重复报告和无关公告。

每个字段族按成本从低到高执行并在覆盖充分时短路：

1. 复用 document/section/parser/schema/catalog/policy 身份均未变化的已完成结果；
2. 使用已有结构化事实。只有带已批准官方证据或单独晋升的官方结构化源可直接完成字段族，
   普通聚合源 candidate 仅用于缩小产品、标题和表格搜索范围；
3. 对 PDF 建立内容寻址的原生页文本和表格制品，使用标题别名、表头签名、关键词和有限上下文
   窗口做确定性解析；
4. 仅把仍未解决的少量页段按兼容字段族批量送入公共 LLM 网关；
5. 对可自动晋升的语义事实执行独立验证，确定性表格事实不重复调用模型；
6. 候选先落治理表，再由版本化系统审核人通过乐观状态迁移和不可变 audit 自动晋升。

人工不再是正常吞吐环节。OCR/上下文/schema/网关等可恢复问题进入 `machine_rework`；只有
有限别名或实体选择进入 `quick_review`；披露冲突、主体或合并范围不清、复杂重组进入
`deep_review`。模型不得直接决定上下游标签、商品方向、未披露材料性、价格传导率、套保
有效性或 DCF 参数。

公司商品暴露拆分为四层：披露事实、产品/商品/行情映射、单独治理的经济假设、以及由兼容
approved 组件构建的发布投影。`company_commodity_exposures` 不再混合承担事实、映射和假设
三种职责；未知材料性保持未知，基础收入端/投入成本端方向只能由显式版本化角色规则推导。
