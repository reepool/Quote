...
| `dce_all` | `["DCE"]` | `["all"]` | 大商所全品类，依赖 Chrome/nodriver 官方源链路 |
| `ine_all` | `["INE"]` | `["all"]` | 原油、低硫燃料油、20 号胶，验证上期系独立交易所日历 |
| `shfe_all` | `["SHFE"]` | `["all"]` | 上期所全品类，适用于交易所整体日更 |
| `czce_all` | `["CZCE"]` | `["all"]` | 郑商所全品类，适用于交易所整体日更 |
...
- DCE 官方接口有瑞数动态 token 防护，普通 `requests`、静态 Cookie、Playwright/headless Chromium 均不能作为生产主路径；工程实现隔离在 `DceOfficialBrowserClient`，要求真实有头 Chrome，服务器环境使用 Xvfb。浏览器启动后会尝试 `maxTradeDate` 作为 warm-up 探针，但该探针失败不直接阻断，最终以实际请求的 `dayQuotes` 或 `contractInfo` 结果为准。若页面内 `fetch` 返回 `HTTP -1 / TypeError: Failed to fetch`，说明当前 Chrome 会话未正确穿过官网 JS 环境，client 必须关闭当前会话并重启后再重试，不能复用坏 session 扫完整个日期区间。异步调度/Telegram 任务环境中不能直接调用该同步 browser client，必须通过 `OfficialFuturesMarketDataProvider` 的 DCE 专用单线程 executor 进入，确保 Chrome 会话、nodriver loop 与请求线程一致，避免 `This event loop is already running` 或跨线程关闭问题。生产环境若无持久 `DISPLAY`，`virtual_display=auto` 会启动 Xvfb；为避免 `-displayfd` 管道等待导致长时间卡死，默认设置 `xvfb_disable_displayfd=true` 并通过 `xvfb_retries` 控制启动重试，日志必须输出 Chrome 路径、display 模式、browser args 和 Xvfb 启动结果。DCE `dayQuotes.tradeType="0"` 表示期货、`"1"` 表示期权；非交易日会返回 `contractId=null` 的“总计”行，必须过滤后再判定交易日。
...
- 各交易所分别实现 adapter，但治理编排必须共用同一接口。第一阶段已落地 DCE、GFEX 与 SHFE；INE、CZCE 后续在调试各自官方源时逐个补充，不允许新增交易所专用旁路任务。
...
该配置的日更执行顺序为：先执行官方交易日历前置回填，再由 `futures_trading_day_governance` 根据已落库的 GFEX/DCE/SHFE/INE/CZCE 官方交易日历生成目标交易日。回填第一步读取各交易所自己的《休市安排》公告，把「X日至Y日休市」写成 `official_holiday_notice` 休市日，夜盘暂停只记在交易时段上，不把当天标成休市。公告休市日不再请求官方日行情，也不进入目标交易日。公告没有覆盖的工作日仍用官方日行情确认；过了发布截止时间仍无文件保持未决，不能把 404 或空文件猜成休市。若没有目标交易日，例如休市日，则后续行情同步自然跳过；若存在目标交易日，则 `futures_master_governance` 仅针对这些目标交易日刷新/发现对应交易所合约主数据；最后 `futures_market_data_sync` 以同一目标交易日集合更新日线和连续序列。多交易所日更必须按交易所隔离前置失败：若某一交易所官方交易日历前置回填 blocked，例如 DCE 浏览器/Xvfb 本地启动失败，应只剔除该交易所并发送 warning 报告，其他已通过前置治理的交易所继续完成主数据治理和行情日更；只有目标范围内所有交易所均 blocked 时，才整体终止任务。历史回补默认同样会执行主数据治理，显式传入 `start_date/end_date`，并将 `master_governance_max_days` 设为足够覆盖本次回补窗口或置空，避免只治理窗口前若干天。`requires_master_data_governance` 是行情日更/回补命令的前置治理开关，不属于 `futures_master_governance` 自身；手工执行主数据治理时不要传该参数。DCE 官方历史行情中存在少量开高低和成交字段为 0、但收盘价/结算价有效的交易日，行情质量标记应使用 `official_zero_ohlc_with_settlement`，不应混同为一般 `ohlc_inconsistent`。
...
主数据治理应在交易日历落库并复核后执行。当前 `futures_master_governance` 已抽象为“单交易所 + 官方日行情合约发现”流程：按目标交易所读取已验证交易日历，按 `exchange + trade_date` 请求官方日行情，使用本地/已 promotion 根品种主数据映射真实合约，并写入 `futures_contracts`。GFEX、DCE、SHFE、INE、CZCE 均复用同一治理流程；交易所差异只允许落在 parser 或 discovery/enrichment adapter，不应另建旁路任务。官方日行情 parser 必须先过滤交易所统计行、期转现/EFP 等非标准期货合约行，再进入主数据发现和行情构造，避免把非上市期货产品误判为 unknown variety。官方日行情仍作为真实合约代码、首次/最后观测交易日的证据，并在 metadata 中保留 `first_observed_trade_date`、`last_observed_trade_date` 和日行情无法直接证明的字段说明；`contract_multiplier`、`tick_size` 等产品规格应由已治理的 `futures_instruments.metadata.contract_spec` 或 discovery/enrichment evidence 下沉到对应 `futures_contracts`，下沉成功的合约质量标记为 `official_daily_discovered_with_product_spec`，确实缺少产品规格证据时才标记为 `official_daily_discovered_partial`。主数据发现层已新增标准化 `FuturesProductSpec` enrichment 接口；DCE 通过既有 Chrome/nodriver 会话访问官方 `/dcereport/publicweb/tradepara/contractInfo`，可提取品种名称、合约交易单位和最小变动价位；同时会从 `config/11_futures.json.master_data_discovery.adapters.DCE.listed_products_page` 指向的大商所首页自动发现“上市品种/合约规则”页面 URL，并可用 `product_rule_pages` 显式覆盖或补充，复用同一浏览器会话解析交易品种、交易代码、报价单位、交易单位和最小变动价位。SHFE 通过官方产品页 adapter 处理 Safeline challenge 与 meta-refresh，发现 `/products/futures/.../<symbol>_f/` 叶子页，并从页面 `pageList` 合约对象解析 `Product`、`ContractSize`、`PriceQuotation`、`MinimumPriceFluctuation`、`ContractSymbol`。CZCE adapter 会先尝试访问官网 `listed_products_page`，静态 requests 遇到 WAF/challenge 时复用浏览器辅助页面 client，等待短 challenge shell 跳转完成后解析 `/cn/sspz/.../H077002...` 上市品种入口，记录官方产品入口 URL、标题和上下文证据；规格字段仍以官方 `FutureDataReferenceData.xml` 为主源，解析 `PrdCd/Name/CtrSz/TckSz/MsrmntUnt/TrdCcyCd`，它能稳定提供品种名称、代码、币种、报价单位、合约乘数和 tick size，但不提供项目内部 `category` 字段；CZCE category 以官方中文品种名和官网入口上下文为输入，通过通用商品分类关键词治理规则推断，并在 field evidence 中标记为 `governed_rule_metadata`，不能伪装成交易所原始分类字段。若当前 XML 不再返回历史根品种，例如 `LR`，则用 `known_products` 保存已审核历史元数据。上期所与上期能源页面和日行情源存在同系混合返回的可能，provider 解析层必须先根据默认注册表、已治理主数据、`known_products/product_rule_pages/category_rules` 构造 root symbol 到交易所的归属映射，再按请求交易所过滤有效行，不能把 INE 品种误写成 SHFE instrument。`contractInfo` 中的 `unit` 是合约交易单位，不是报价单位，不能直接写入 `candidate_unit`；若 DCE 上市品种页面提供“报价单位”，该页面字段可作为 quote unit 的官方证据，否则报价单位和分类通过 `known_products` 的显式治理规则元数据补齐。主数据治理可按交易所配置使用同一 adapter 刷新已存在的 P0 根品种，并按已有 root symbol 定向请求官方规格，避免无目标抓取造成跨交易所产品污染；DCE 日常任务默认跳过既有品种规格刷新，以保留短效代理容量，但日行情发现未知品种时仍执行定向官方 enrichment，其他交易所维持既有刷新行为。只有字段确实变化，或拿到官方 product spec 证据时才更新 `futures_instruments/futures_series`，单纯复读内置 seed 不触发无意义写入；报告必须披露 `initial_instruments/final_instruments/refreshed_instruments` 与对应 series 计数。若官方规格 enrichment 源不可用，例如 DCE 浏览器会话失败或 SHFE/CZCE 页面 challenge 无法解开，任务不能只写日志，必须在 Telegram/report warnings 中披露 `official_product_spec_enrichment_unavailable`、交易所、目标 symbols 和错误文本。对 DCE 这类依赖浏览器会话的官方源，单日请求失败不能立即形成永久缺口，任务应在全段扫描结束后按 `master_data.contract_discovery_retry` 进行任务级补跑，最终报告必须披露 `task_retry_passes`、`task_retry_resolved`、`failed_trade_dates`。

未知品种 discovery 已覆盖所有已配置交易所。当前静态 P0 根品种种子会作为内置 enrichment 元数据，交易所新增品种若未进入 P0 种子，也会在报告中以 `unmapped_<exchange>_varieties` warning 暴露，并生成 discovery 候选；通用 adapter 会优先从官方日行情 raw payload 中提取可用的品种名称证据，再合并交易所规格 enrichment 和 `known_products`。若名称、分类、币种、报价单位等关键字段完整，则可高置信 promotion；若只能拿到名称、合约乘数、tick 等部分字段，则保留 `discovered_verified_partial/pending`，不能写入正式 `futures_instruments`。GFEX 的 `PT`、`PD` 已作为内置补充元数据保留，用于回归验证“正式根品种种子落后时仍能发现并生成候选”的完整链路。DCE 当前已接入官方 `contractInfo` 规格 enrichment，并为 `A/B/BB/BZ/C/CS/FB/JD/LG/LH/M/P/RR/Y` 补充分类与报价单位元数据；全历史 dry-run 发现 2000-2003 年存在旧根品种 `S`（官方日行情名称“大豆”），其是 DCE 早期大豆/黄大豆 legacy soybean 合约代码，后续拆分为 `A` 黄大豆 1 号与 `B` 黄大豆 2 号两条体系。治理上 `CNF.S.DCE` 必须作为独立历史根品种入库，不映射为 `A` 或 `B`，并在 `metadata_json.master_discovery_evidence.product_lineage` 中记录 `successor_family=["CNF.A.DCE","CNF.B.DCE"]`、`primary_chronological_successor="CNF.A.DCE"`、`oilseed_import_soybean_successor="CNF.B.DCE"`。CZCE 当前已通过官方 XML 和治理规则覆盖 AP/CF/CJ/CY/JR/LR/OI/PF/PK/PL/PM/PR/RI/RM/RS/SR/WH 等新增或历史根品种，其中 `LR` 作为当前 XML 不返回的历史品种由 `known_products` 提供 name/category/currency/unit/multiplier/tick。所有候选在官方规格与配置规则都可用时可自动 promotion。`futures_master_governance write` 在同一任务内完成 auto-promotion 后，会复用本轮已抓取的官方日行情 rows 重处理 newly promoted varieties，因此无需额外再跑一遍才能写入这些新品种的真实合约。SHFE、INE、CZCE 均已按同一 `FuturesProductSpec` 标准接口接入各自官方产品规格或交易参数源。

CZCE 全历史治理还会遇到当前 XML 不再返回的 `TC` 历史根品种。`TC` 来源于 CZCE 官方历史日行情中的动力煤历史合约代码，按独立历史根品种治理，使用 `known_products` 提供 name/category/currency/unit/multiplier/tick 证据，不作为当前 `ZC` 的行情替代写入。

真实合约的 `contract_month/delivery_month` 不能只靠合约代码做无上下文静态推断。交易所 adapter 或主数据治理层必须保留交易日上下文，必要时用 `first_observed_trade_date` 辅助判定合约年份。CZCE 历史日行情存在 `TC511`、`AP001` 这类三位年月代码：同一个末位年份在不同年代会重复出现，治理时必须在候选年份中选择与首次观测交易日最匹配的合约年月，避免把 2015 年合约误写为 2025 年合约。

新增未知品种的长期目标是“尽量自动入库、少人工干预”。所有交易所的 discovery adapter 必须实现统一的产品主数据 enrichment 接口，输出标准化 `name/category/currency/unit` 以及可选的 `contract_multiplier/tick_size/lineage` 证据。字段来源优先级为：交易所官方产品/合约规则接口、官方日行情 raw payload、交易所公告或规则页面解析、已审核本地规则补充、聚合源低质量兜底；不得用不透明猜测直接写正式主数据。日更或回补遇到 unknown variety 时，应先调用 enrichment adapter，若 `name/category/currency/unit` 均达到可信质量，则自动写入 `futures_master_discoveries` 并 promotion 到 `futures_instruments`、`futures_series`，随后复用本轮官方日行情 rows 写入对应真实合约；若仍缺关键字段，则写入 pending review 并在报告中明确列出缺失字段、来源、影响的合约和是否跳过行情写入。当前 DCE、GFEX、SHFE、INE、CZCE 均已具备交易所专用官方产品规格路径：INE 复用上期系产品页结构，并在静态 requests 被 challenge 阻断时通过浏览器辅助官方页面 client 解析 Safeline/meta-refresh/pageList；CZCE 先通过浏览器辅助官网首页/上市品种入口发现产品页面，再用官方 `FutureDataReferenceData.xml` 作为结构化一手规格源解析 `PrdCd/Name/CtrSz/TckSz/MsrmntUnt/TrdCcyCd`。通用产品页 parser 只能作为基础工具，不能替代交易所具体实现。项目内部 `category` 属于已审核治理规则，不伪装为交易所原始字段，必须在 field-level evidence 中标明为 `governed_rule_metadata`。任务发现新品种后只写 `futures.db`，不会自动回写 `config/11_futures.json`；Telegram 报告必须明确提示应维护的文件、JSON path（例如 `master_data_discovery.adapters.DCE.known_products.BZ`）和建议 entry，便于人工复核后把长期治理规则沉淀到配置。若官方产品页或 XML 被 WAF/challenge 阻断，任务必须报告 `official_product_spec_enrichment_unavailable`，并让缺字段候选保持 pending，不允许绕过统一 discovery/promotion 流程。
