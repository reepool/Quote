...
- **`task_async_pool.pool_size`**: `int` (默认: `8`) —— *后台任务专用异步连接池基础连接数；当前与公告解析、行情回补等后台 I/O 并发上限匹配*
...
> API 与任务使用独立的 SQLAlchemy `AsyncAdaptedQueuePool`。FastAPI 普通请求通过 middleware 标记为 `api` workload；调度器、数据维护、CLI/手工任务，以及管理 API 触发的数据生产任务默认或显式使用 `task` workload。不得给异步引擎配置同步 `QueuePool`。该隔离保护的是连接池容量；SQLite 文件级写锁仍需通过短事务和任务串行控制。
## data_config
...
- **`trading_status_announcement_scan_enabled`**: 是否通过统一公告接口扫描 HKEXnews 标题分类。`Trading Halt` / `Suspension` / `Resumption` 补短暂停牌和长停复牌：最新停牌写入 `status=suspended,trading_status=0,source=hkexnews_trading_halt`；最新真实复牌写入 `status=active,trading_status=1,source=hkexnews_trading_resumption`。标题写明未来 `resume` / `commence` 日期时，复牌日未到仍保持 `suspended/trading_status=0`，并保留复牌证据；只有 `as_of` 达到该日期才写回可交易。停牌与复牌、以及月度长停牌 PDF 的 `as at` 日期同时存在时，日期靠后的证据覆盖靠前的。标题仍是 continued suspension、没有复牌动作词的，不按复牌写。同一开关还会扫交易安排、资本重组、介绍上市、撤回上市和 CIS/ETF 事项：只在当前不可交易窗口内写 `status=active,trading_status=0`（`hkexnews_trading_arrangement` / `hkexnews_product_cessation`），到期后恢复 `trading_status=1`。没有明确复牌日的产品停运（`hkexnews_product_cessation`）视为持续不可交易，不因 8 个月扫描窗口滚出而恢复；证券主列表/HKEXnews active 或更早的复牌证据不能把它清掉。人工复核 `action=active` 具有最高优先级，后续正式复牌（`hkexnews_trading_resumption`，证据时间不早于停运）或已到期的 `expected_resume_date` 可以结束该状态。带明确复牌/开盘日的交易安排仍按窗口到期恢复。CIS 只落到基金/ETF 代码，双柜台终止不误伤剩余 HKD 柜台。撤回上市只认标题里已经停止买卖或已决定取消上市的表述，不看栏目名里的 Cancellation of Listing，也不把拟私有化月度更新写成停买。同一条公告夹带票据/窝轮时不写正股。空行情本身不会改生命周期。公告扫描关闭、跳过、失败、降级或分页未完成时，不得把已有 `trading_status=0` 写回 1，也不得用残缺扫描驱动新的资格写入。只有成功且完整的扫描（含合法空结果）才允许恢复可交易。官方退市列表优先于停复牌扫描残留的 active 行；同一代码同时出现在证券主列表和退市列表时进入人工复核，不自动退市。当前仍有效的交易安排/产品停运行带顶层 `lifecycle_evidence_at`，合并时不被更早的复牌或 listing 行清掉。`mark_instrument_untradable` 只写入合并后仍为有效不可交易窗口的官方行，不回写已被复牌或人工 `active` 覆盖的旧资格行。
- **`trading_status_lookback_months`**: 标题扫描回看月数。HKEXnews 全市场查询大约只能覆盖一个月，provider 会按自然月切窗，不要在业务层再写一套抓取。该窗口用于发现仍在公告期内的短暂停牌和有明确到期日的交易安排；它不是产品停运的失效条件。
- **`trading_status_scan_max_pages`**: 每个市场 × 自然月窗口最多请求的标题页数。HKEXnews 用累计 `rowRange` 做 Load More，并读取 `hasNextPage` / `hasNextRow` / `recordCnt`；达到页数上限仍有下一页时标记扫描不完整。
...
- **`resource_protection.reserved_task_connections`**: `int` (默认: `8`) —— *任务侧连接容量目标；当前生产通过独立 `task_async_pool` 物理保障*
