...
- **`trading_status_announcement_scan_enabled`**: 是否通过统一公告接口扫描 HKEXnews 标题分类。`Trading Halt` / `Suspension` / `Resumption` 补短暂停牌：最新停牌写入 `status=suspended,trading_status=0,source=hkexnews_trading_halt`；最新复牌不写停牌行，官方 active 列表可按原规则 `mark_instrument_active` 复活。同一开关还会扫交易安排、资本重组、介绍上市、撤回上市和 CIS/ETF 事项：只在当前不可交易窗口内写 `status=active,trading_status=0`（`hkexnews_trading_arrangement` / `hkexnews_product_cessation`），到期后恢复 `trading_status=1`。CIS 只落到基金/ETF 代码，双柜台终止不误伤剩余 HKD 柜台。撤回上市只认标题里已经停止买卖或已决定取消上市的表述，不看栏目名里的 Cancellation of Listing，也不把拟私有化月度更新写成停买。同一条公告夹带票据/窝轮时不写正股。空行情本身不会改生命周期。
