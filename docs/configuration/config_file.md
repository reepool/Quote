...
- **CNIndex 主键语义**: `指数代码` 不是本地行情主键。只有官方列表提供 `深交所行情代码` 时，指数才写入行情型 `<行情代码>.SZ` 并参与日更；没有行情代码的 CNIndex 行写为 `.CNI` 或 `CNI<指数代码>.SZ` metadata-only 身份，保留主数据但不触发每日行情空抓，也不得覆盖同代码股票。治理前快照或本地已存官方 metadata 证明无行情代码时，同源、同 6 位代码的遗留行情型 key 会被标记为 `metadata_only` 并从日更 universe 排除。
