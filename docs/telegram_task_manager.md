...
股东摘要当前没有单独的 Telegram 专用命令，使用通用 `/run shareholder_shadow_sync` 手工触发全量刷新，使用 `/run shareholder_reconciliation_sync` 触发周期复核与补足，使用 `/run shareholder_incremental_sync` 触发公告驱动增量检查。全量刷新任务在 scheduler 中配置为 `manual_only=true`，不会自动注册为周六定时任务，但仍会出现在 `/status` 中，状态为仅手工执行入口；周期复核默认周六 `12:30` 执行，全量读取后通过与日更相同的归一化内容 hash 和 required scope 判断，只写变化、本地缺失或覆盖不完整标的；每日增量检查默认 `06:30` 执行。增量检查按 CNInfo 定期报告分类流和权益变动窄扫描发现候选标的，再做结构化股东数据 hash 比较，只有变化或 required scope 缺失才写入快照；`max_candidates=0` 表示当日候选全部处理完。公告先到、data20 结构化数据滞后的候选会进入 5 个自然日 pending recheck，同一批公告的截止时间固定为第一次进入 pending 的时间加 `pending_recheck_days`，不会滚动延长。配置变更后使用 `/reload_config` 可刷新同进程内的 scheduler 与 `DataManager` 运行时配置；如果 API 是单独进程，需要重启 API 进程才能加载修改后的 research 配置。
...
| `shareholder_reconciliation_sync` | 周六 `12:30` / `/run` | `SSE / SZSE / BSE` 全量活跃股票 | `changed_only`，与日更相同的归一化内容 hash 和 required scope 相同则跳过 |
...
| 任务 | 触发方式 | 读取范围 | 写入策略 |
|---|---|---|---|
| `financial_l1_full_import` | 仅 `/run` | `SSE / SZSE / BSE` active 股票池和最近 rolling 报告期 | 可续跑：`resume=true` 且未指定 `log_dir` 时复用最近未完成目录；`needs_review` 批次下次会重试未齐标的；已完整落库的标的/报告期跳过 |
| `financial_disclosure_incremental_sync` | 每日 `21:45` / `/run` | CNInfo 四类定报 category、披露异常关键词窄流、BSE 官方定报 subtype、pending recheck、本地缺失报告期 | 当天处理全部公告候选，不再按数量截断；扫描未完成时降级且不推进游标；先用 CNInfo data20，缺口再用 THS/Sina |
...
日更候选只包含新公告和固定重试截止时间内的 pending 状态；`accepted_disclosure_gap` 留给周度对账复核，不再每天占用候选上限。业绩预告、预增/预减和业绩快报也按非正式定报过滤。超过截止时间仍未就绪的 pending 转为 `pending_recheck_expired`，保留原公告和时间证据。CNInfo 官方结构化补数出现路由错误时，即使 fallback 成功，任务状态仍为 `degraded`，报告会同时显示 fallback 写入结果和后续官方对账提示。
...
报告期结束日早于股票上市日的本地缺口或历史 pending 会显示为 `accepted_gap/pre_listing_period`，不会进入 CNInfo/THS/Sina 补数源统计；这类数量增加不是 CNInfo 失败，而是主数据生命周期解释生效。
...
- **主数据报告**: 日更报告包含“证券主数据同步”段落，展示新增、停用、活跃数量、来源和 warnings/errors；该段落来自共享 `instrument_master_governance` 治理入口，但保留日更兼容字段 `instrument_master_sync`。A 股股票主数据按 `exchange_official -> BaoStock -> AkShare` 路由，官方源失败时会在报告中降级为 `baostock_fallback` 或 `akshare_fallback`；HKEX 使用专项 lifecycle_write 治理，报告中的 `source=` 来自 `source_usage`，会显示 `hkex_securities_list`、`hkexnews_active_list`、`hkexnews_delisted_list` 等官方来源组合。历史补数模式默认跳过当前主数据同步，避免用今天的股票池语义污染历史回补。午夜后重跑前一交易日例外，例如 2026-06-16 00:18 执行 `/run daily_data_update 2026-06-15` 仍属于当前日更重试，应强制运行 A 股股票和指数主数据治理。
- **无行情指数**: 指数治理会把 CNIndex 官方列表中没有 6 位纯数字 `深交所行情代码`、只有 `.CNI`/CNI 主数据编号的指数保存为 metadata-only 身份；本地遗留的错误行情型 key（如 `005125.SZ`、`005126.SZ`、`006125.SZ`）和非 6 位 `.SZ` key（如 `39926401.SZ`）会被标记为 `metadata_only` 并从 `tradable_only` 日更 universe 排除。Telegram 报告中的指数主数据治理段会展示 metadata-only、invalid-quote 遗留停用数量和少量样例；若日志仍看到这些代码被行情源反复空抓，优先检查当次任务是否跳过了指数前置治理。
- **指数官方行情源**: 普通 A 股指数日线按指数族使用官方源优先，`SSE index = CSIndex -> BaoStock -> AkShare`，`SZSE index = CNIndex -> BaoStock -> AkShare`。官方源返回非空但最新行情日早于请求区间内最后一个交易日时会继续 fallback，避免把 stale 数据误判为成功；同一轮同源同目标交易日连续 stale 达到配置阈值后会临时熔断该官方源，后续指数直接走 fallback，降低 CNIndex/CSIndex 批量晚更新时的重复请求。主循环结束后会对仍未覆盖的失败代码按 `daily_update_unresolved_retry` 再请求配置源（默认官方主源，忽略本轮熔断）；挽回数量出现在 Telegram「完整性」的「补拉」。
...
- **历史修复治理**: `/find_gap_and_repair`、月度完整性检查和区间回补会先执行本地 `repair_universe_governance` 过滤；停编指数、退市后窗口、metadata-only/stale-no-quote 指数，以及 HKEX `status=suspended,trading_status=0` 且请求区间晚于本地最新行情日的长期停牌股，会在行情源请求前跳过或裁剪。A 股指数优先按 `index_lifecycle_evidence.last_quote_date` 裁剪，缺失最终行情日时才退到 `effective_date`，本地 stale-no-quote 或短 delisted-date 规则会以 `*_fallback` reason 和 `degraded_fallback_count` 显示为降级兜底。HKEX active/tradable 但缺少 `listed_date` 的当前标的，会用本地首个行情日作为检测起点，避免把上市前或入库前窗口算成缺口。周度 `/find_gap_and_repair` 会在检测前仅对 `hkex_instrument` scope 执行 HKEX lifecycle 前置治理，不触发 A 股全市场刷新。报告的“生命周期过滤”段落会展示数量、原因、裁剪原因和样例。对本地完全没有行情记录但仍为 active 的指数，系统不会直接判定停编，而是在 `/find_gap_and_repair` 中按 `max_index_no_data_failures_per_instrument` 做指数连续无数据失败熔断，避免单次任务反复请求外部源。
...
• 海外特殊商品日频价格同步 (special_commodity_overseas_daily_price_sync) - 周二至周六 08:00
• 大宗商品非价格产业指标聚合同步 (special_commodity_industrial_indicator_sync) - 周一至周五 16:30
• 国内特殊商品现货价格与基准同步 (special_commodity_domestic_spot_price_sync) - 周一至周五 22:30
• 国际特殊商品月频价格基准同步 (special_commodity_international_monthly_price_sync) - 每月10日、20日 08:40
• 特殊商品政策目录发现与事件治理 (special_commodity_policy_governance_sync) - 每月15日09:10自动 write；发现并保存证据/候选、对账正式事件，但不自动批准新政策
