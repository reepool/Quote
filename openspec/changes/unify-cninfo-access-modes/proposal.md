...
- Route by host/path, not by domain provider: allowlisted first-party www APIs may use headed Chrome; unallowlisted www uses the old TLS stack; `webapi.cninfo.com.cn` and `http` never go through headed Chrome. HTTPS `static.cninfo.com.cn` attachments attach the mux; headed-first static PDF is owned by `cninfo-headed-static-pdf-access`. AkShare `*_cninfo` wrappers stay outside the mux.
...
- Do not replace shareholder / announcement / filing parse or write owners. Do not wrap AkShare `stock_hold_*_cninfo`. Do reuse `attach_cninfo_access` for default CNInfo attachment sessions. Headed-first static PDF is owned by `cninfo-headed-static-pdf-access`. Do not reuse `DceOfficialBrowserClient`. Do not build a project-wide browser platform. Do not grow `data_manager.py`, `scheduler/tasks.py`, `research/storage.py`, or `api/routes.py`.
...
- No scheduler job id, write owner, canonical table, or AkShare wrapper change. Default CNInfo attachment HTTP joins the mux; XDXR / announcement-asset / broker-risk callers inherit it through `AnnouncementAttachmentRetriever`.
