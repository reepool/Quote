...
- Close the production failure found for `PCS`, `平方`, and `立方`: resolve these governed aliases deterministically, serialize every unit-proposal request as JSON-safe data before gateway admission, and expose bounded proposal-failure diagnostics so a transport attempt cannot be confused with a local payload-construction failure.
- Close the next production-observed gaps for `亿吨千米` and malformed table-header unit `元币种：人民币`, keep deterministic parser facts out of the semantic verifier, recover identical duplicate primary-key bundles automatically, and make unit notifications use `✅` only for the final verified effective state and `⚠️` otherwise.
- Raise structured-shadow PDF parse concurrency from four to eight on the current 16-core host while retaining work-conserving refill, bounded queue claims, immutable page-artifact reuse, and the single SQLite writer; defer an alternate PDF engine until a fidelity benchmark proves equivalent extraction.
