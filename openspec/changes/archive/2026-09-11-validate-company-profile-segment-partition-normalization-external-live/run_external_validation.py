"""Run the two-case segment partition normalization validation externally once."""
...
CONTRACT_PATH = Path(__file__).with_name("external-validation-contract.v1.json")
EXPECTED_VALIDATION_ID = (
    "manufacturing-materials-shadow-segment-normalization-external-live-20260911-b"
)
EXPECTED_CASES = (
    (
...
def _validate_contract_payload(contract: dict[str, Any]) -> None:
    if contract.get("schema_version") != (
        "company_profile_segment_partition_external_live_validation_contract.v1"
    ):
    ...
    if contract.get("execution_path") != "sandbox_external_authorized":
        raise ValueError("external execution path mismatch")
    authorization = contract.get("transfer_authorization")
    if authorization != {
        "target": "scorpio.reepool.com",
        "scope": "only frozen Evidence and request content for the two contracted scopes",
    }:
        raise ValueError("transfer authorization mismatch")


def _validate_predecessor(contract: dict[str, Any]) -> None:
    predecessor = contract["predecessor_validation"]
    path = REPOSITORY_ROOT / predecessor["path"]
    if _sha256(path) != predecessor["sha256"]:
        raise ValueError("predecessor validation hash mismatch")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("validation_id") != predecessor["validation_id"]:
        raise ValueError("predecessor validation identity mismatch")
    if payload.get("provider_calls") != 2 or payload.get("upstream_responses") != predecessor["required_upstream_responses"]:
        raise ValueError("predecessor execution result mismatch")
    if not all(case.get("classification") == predecessor["required_classification"] for case in payload.get("cases", [])):
        raise ValueError("predecessor case classification mismatch")
...
def run(contract_path: Path = CONTRACT_PATH) -> dict[str, Any]:
    ...
    try:
        ...
        result = {
            "schema_version": "company_profile_segment_partition_external_live_validation_result.v1",
