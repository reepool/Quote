...
5. **翻页策略**：`count = batch_size = 800`，端点实返 **800+1 根**，按日期去重；从窗口 `end_date` 向前取，下一页 end 边界 = 当前页最早 bar 日期 − 1 天；终止条件：最早 bar（取**页内日期最小值**，不依赖端点返回顺序）≤ 回看起点、页返回空（**含 end 早于上市日的正确空**）、或安全阀 `max_iterations = 100`（与 tdx 相同）；不以「返回根数」判断最后一页。跨页按日期去重、升序输出。pre_close 回看使常规日更 `[T-1, T]` 每票约 1–2 个请求。
...
8. **失败语义（对齐 harden-a-share-daily-source-failover）**：数据级失败（空节点/死代码/畸形行）→ 返回 `[]` + `last_fetch_diagnostic`，不抛异常；传输级故障（连接拒绝/重置/超时）→ 抛 `ConnectionError`（被工厂 `_is_daily_source_unavailable_error` 的 isinstance 分支命中）。**HTTP 状态码二分**：403/429 抛纯 `TencentHTTPStatusError`（带 `code`/`status` 属性，被 `_is_daily_http_throttle_error` 识别），立即上抛不重试，只归 throttle 熔断；5xx 等传输级状态码抛 `TencentHTTPTransportStatusError`（继承内置 `ConnectionError`，被 `_is_daily_source_unavailable_error` 命中）计入 transport 熔断——两类均不吞成 `[]`。网络错误与 5xx 按 `retry_times`/`retry_interval` 在源内有界重试（仅传输级；重试 sleep 在 to_thread 工作线程内，不属于日更主循环的禁止退避）。健康空结果**不**置 `connection_unhealthy`。日更主循环内**禁止**失败退避 sleep（`RateLimiter.acquire()` 的 pacing 等待不属于此列）。
...
10. **限流**：走基类 `RateLimiter`，`data_sources_config.tencent` 与 `RateLimitConfig` 同名字段：`max_requests_per_minute: 3600`、`max_requests_per_hour: 60000`、`max_requests_per_day: 1000000`。推导：新端点串行实测 ≈11 只/s ≈ 660 次/分，3600/min 约为 5 倍余量；并发 54.5 只/s 仅说明容量上限充裕。**限流按 HTTP 页计数**：`acquire` 在异步翻页循环内逐页执行，跨年回补一只票多页时每页各占一个 token。全市场一次串行回退 ≈9–13 分钟，在限额内。
