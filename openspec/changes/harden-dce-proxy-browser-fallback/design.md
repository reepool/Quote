...
12. **Rotate short-lived routes immediately and reuse daily evidence across phases.** Proxy responses such as `HTTP 403 ip expired` and route-local `request too frequent` mean the current exit is no longer usable; refreshing that browser session or applying the provider's generic minute-scale retry cannot recover it. The DCE browser client therefore rotates immediately within its existing lease bounds and remains the sole retry owner. The task-scoped official provider caches successful `dayQuotes` payloads by normalized trade date so calendar, master, and price phases consume the same official evidence without spending additional short-lived proxy capacity. Routine governance skips DCE product-page enrichment for existing governed roots, while a newly observed variety still performs targeted official enrichment.
...
- **[A proxy lease expires between successful requests]** -> Recognize explicit expiry or route throttling, record credential-free metrics, and rotate immediately instead of refreshing or entering generic provider backoff.
- **[Task-scoped payload reuse could retain stale data]** -> Scope the cache to one provider/task instance and key it by trade date; a new scheduled or manual task always creates a fresh cache.
