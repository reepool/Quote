...
历史四报告、六章隔离切片只描述当时的 Stage 5 验证，不作为当前准入合同。阶段 3 仍把转型、重大重组或反向上市样本记为阻塞性覆盖缺口；本 change 不靠补一家未核验发行人来关闭该缺口。
...
- 样本下限使用研究合同已点名的 `300750.SZ` 2025、`603659.SH` 2025、`920015.BJ` 2025。它们分别位于 SZSE、SSE、BSE，并且不是当前固定两家公司。
...
1. **样本沿用阶段 3 已点名组合，不重新选样。**
   三份报告、两家以上公司、两份挑战报告和三个交易所已经由研究合同写明。本 change 不另挑公司，也不把固定两家公司加进这组样本来凑数。dossier 必须自行记录披露形态和业务模式；如果三份报告的披露形态或业务模式仍然过窄，记覆盖缺口，不假装已经覆盖。
...
3. **只选一个已有章节任务。**
   dossier 完成后，从现有章节任务里选一个最小竖切。选择必须有至少两家不同公司的 dossier 支持，并且不能靠行业常识补名称。未入选的五章保持关闭。历史六章包不因此重新启用。
...
- [过早选定竖切] → 1.1 只接受选择规则；具体章节任务在三份 dossier 完成后才命名。
