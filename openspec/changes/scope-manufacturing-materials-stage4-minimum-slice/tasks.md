## 1. Scope review gate

- [ ] 1.1 Independent review accepts sample diversity, the one-slice rule, field boundaries, research isolation, and the later verification method. Do not change Python, identity, publication, closure, mode, or checkpoints until 1.1 is checked

## 2. Implementation after review, not yet authorized

- [ ] 2.1 Write an independent dossier for each approved report before choosing a slice
- [ ] 2.2 Name exactly one existing chapter task that two company dossiers support, and leave the other chapters inactive
- [ ] 2.3 Implement that slice only by reusing Evidence, Stage 5 extract/repair/verify, and the research isolation bundle, with output limited to `accepted_for_review`

## 3. Focused replay, not yet authorized

- [ ] 3.1 After implementation review, replay only the approved reports for the selected slice
- [ ] 3.2 Independently recount recall, accuracy, critical numeric errors, and expansion gates. Do not presume they pass, and do not reuse the fixed two-company 9/9
- [ ] 3.3 Do not start another expansion, authorize scale quality or production, enable the six-chapter package, or open DCF, trading, or the legacy writer
