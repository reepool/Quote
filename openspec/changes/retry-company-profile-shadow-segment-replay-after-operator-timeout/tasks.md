...
- [ ] 3.1 Run one read-only Scorpio/Gemini preflight through the authorized path.
- [ ] 3.2 Run the new twenty-report cohort exactly once without an external hard
  cutoff, preserving every report-local success or typed failure.

## 4. Produce empirical artifacts

- [ ] 4.1 Generate source-review outcomes for all six core chapters and record the
  interrupted predecessor as excluded execution failure.
- [ ] 4.2 Generate segment-recurrence, readiness, and immutable baseline
  comparison audits from the new batch only.

## 5. Close and verify

- [ ] 5.1 Record `ready`, `hold`, or `failed` from the existing gates and retain
  `production_authorization=not_authorized`.
- [ ] 5.2 Run focused tests, Ruff, hash/artifact checks, `git diff --check`, and
