...
| 量纲对账 | 手：600000.SH 898172×100=89,817,100、000001.SZ 1105134×100=110,513,400、300750.SZ 262864×100=26,286,400、920000.BJ 11810×100=1,181,000（均与库内 pytdx 行精确一致）；股：688981.SH 22,153,826、688111.SH 4,181,985、689009.SH 7,250,217（原始值即股） |
...
3. **量纲（按代码前缀，不按交易所板块标签）**：`sh688*` / `sh689*` → 原始值已是股（×1）；其余本源覆盖的 A 股股票（SSE 主板、SZSE 主板/创业板、BSE）→ 手，×100 转股。依据为 7 只票 ×100/×1 双向对账（见 Context 表）。**不采用 AkShare `stock_zh_a_hist_tx` 的 `startswith(("sh688","sz399","sh000","sz000"))` 启发式**——`sz000` 会错杀深主板（000001.SZ 实测必须 ×100）。`get_latest_daily_data` 快照的 idx6 适用**同一条前缀规则**。
