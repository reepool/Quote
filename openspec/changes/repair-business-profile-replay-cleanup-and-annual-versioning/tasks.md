...
- [ ] 10.1 Run read-only scans for as-of visibility, unit anomalies, stale reuse identities, pending rows, action lineage, held records, and lease-risk work before any LLM call.
- [ ] 10.2 Apply cleanup only to confirmed non-reusable receipts/runs/work/candidates and orphan checkpoints; preserve approved records, evidence, and review audits; emit a deletion manifest.
- [ ] 10.3 Re-run targeted `002415.SZ`, `002496.SZ`, and `300750.SZ` flows with `result_policy=reuse`; require zero temporal collisions, false gateway classifications, stale reuse, and duplicate exposure lineage.
...
- [ ] 10.6 Review the implementation against the current HEAD, run focused and business-profile regression suites, and update this change with evidence before archive.
- [ ] 10.7 Update the `Deferred Backlog Register` in `design.md` with the disposition of non-blocking findings (format checker, NFKC normalization, structured sync watermark, large-record pagination, approval supersession persistence, catalog monotonicity, source truncation, GBK handling, and bookmark-title dead path) before archive; each item must have an owner or explicit follow-up change.
