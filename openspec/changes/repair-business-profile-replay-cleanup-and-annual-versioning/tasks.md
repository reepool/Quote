...
- [ ] 5.9 Extend retired-shadow cleanup to semantic runs and durable receipts, using the same mode/marker predicate and proving replay lookup cannot return a retired run after its work row is removed.
...
- [ ] 10.1 Run read-only scans for as-of visibility, unit anomalies (including legacy `mw`/`mW` values), stale reuse identities, pending rows, legacy exposure action lineage, held records and latest hold owners, retired runs/receipts, and lease-risk work before any LLM call.
