...
本 change 以三份已在本地正式公告库验证为 `local_valid` 的 2025 年报启动：宁德时代（SZSE，232 页）、璞泰来（SSE，203 页）、锦华新材（BSE，143 页）。三者覆盖电池系统、材料/加工/装备、精细化工材料以及三种交易所模板。它们均以稳定主业为主，因此转型/重大重组/借壳仍是 blocking coverage gap，必须在行业合同批准前补样。
...
### Decision 3: 初始三样本启动，regime 缺口阻断最终批准

宁德时代用于复杂电池系统；璞泰来挑战多材料、加工、装备和多子公司口径；锦华新材挑战 BSE 模板、精细化工单位和客户供应商披露豁免。三份满足阶段 2 的启动下限，但没有证据证明已覆盖转型/重组，因此可以开始研究、不能批准最终行业合同。

转型样本必须同时具有正式年报和主业变更/重组证据，且与制造/材料边界高度相关。若不存在合适样本，保留 blocker，不用低相关公司凑数。
...
- [Risk] 三个稳定主业样本会低估 regime 复杂度。→ 最终批准前强制补转型/重组样本；未补即 `held`。
...
2. 为三份初始年报建立逐报告 dossier 和初标；并行寻找经正式证据确认的转型/重组样本。
...
- 哪一份正式转型/重大重组报告最能代表制造/材料 regime 切换，需要在样本发现任务中以正式证据决定。
- 产量/销量/库存应成为整个制造/材料包的 required 检查，还是只在监管模板或业务形态触发时 conditional，由三份 dossier 和补充样本决定。
- 设备制造与材料加工是否共享一个第一版包，还是在研究结论中拆成 subtype checklist，由样本证据决定。
