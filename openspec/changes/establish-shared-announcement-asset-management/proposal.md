...
- Add an independent A-share annual-report daily task that discovers recent formal full annual reports and corrections, downloads the effective attachment, and maintains one effective local attachment per instrument and fiscal year.
...
- Add local-first consumer APIs: callers first reuse a verified local asset; when absent, an authorized caller may discover, register, and download the required annual report without depending on business-profile scheduling.
...
- Add independent configuration, scheduler reporting, disk-space gates with concurrent byte reservations, independent-failure-domain file-backup requirements, observability, reconciliation, and rollout controls. The first release covers formal annual reports and corrections; semiannual and other announcement types remain extension points.
