...
Internal batch consumers may wait within configured bounds. API calls create or reuse an asynchronous durable operation and return the caller's opaque `asset_request_id` subscription handle rather than either block indefinitely or expose the internal operation id.
...
A generic asset ensure obtains only the shared source asset. A front-facing business command that depends on a missing asset persists a consumer-specific continuation and enqueues exactly one consumer operation when the asset becomes valid, exposing a caller-scoped `consumer_request_id` separately from the `asset_request_id` projection. Asset completion never implies that business-profile or broker output is already current. Only the owner or operator can query either request projection; internal asset and consumer operation ids remain operator/service details.
