...
Instrument, fiscal-year, exchange, source, and filing identities remain database projections. During shadow migration, a canonical record may reference an existing path, including a broker-held complete original or complete correction. Shadow records remain excluded from production effective lookup, bootstrap coverage, and consumer parsing until source/period/classification/hash/effective-decision reconciliation passes. An independent asset-adoption promotion gate then makes the record production-visible for shared bootstrap and daily reuse; it does not wait for either business consumer cutover. A later verified move, hard link, or copy-verify-rename can converge storage without unnecessary network download. NFS mount identity and filesystem capabilities are probed first. Duplicate paths are deleted only through a per-file manifest/hash allowlist after each affected consumer has switched and reconciliation passes; derived files, semiannual reports, other years, orphans, and conflicts are excluded.
...
Temporary files carry operation, owner, lease generation, creation time, and planned-byte identity. A restarted worker may remove or adopt a stale `.part` only after owner/heartbeat/generation reconciliation and a safety grace period. Quarantine is evidence rather than a cache: it has age/byte readiness thresholds and is cleaned only through an operator-authorized audited command. Releasing a byte reservation does not by itself prove that temporary or quarantine bytes were removed; the two states are reconciled separately.
...
A generic asset ensure obtains only the shared source asset. A front-facing business command endpoint owned by business-profile or broker risk-control is the entry point for starting domain processing: it accepts that consumer's processing fingerprint and caller idempotency key, creates or reuses a `consumer_request_id` immediately, and uses `pending_asset` while a missing asset is acquired. It enqueues exactly one consumer operation when the asset becomes valid. Asset completion never implies that business-profile or broker output is already current. A current result is valid only when it is bound to the current effective shared asset ID/content hash and the same processing fingerprint; a superseded result is stale and must requeue or terminate, never return HTTP 200. Only the owner or operator can query either request projection; internal asset and consumer operation ids remain operator/service details. Generic ensure never starts unrelated consumers. Local-hit/local-miss asset ensures return no `asset_request_id`; only asynchronous acquisition returns one. A business command returns HTTP 200 only for an existing current consumer result; a local asset hit with parser work pending and an asset miss with acquisition both return HTTP 202, with the latter carrying both caller-owned handles. Network-disabled or policy-blocked consumer requests terminate with structured `missing|blocked` projections without provider work. The business command `Location` points to the consumer request; the asset request URL is a body link. Its domain processing permission does not grant `annual_report_assets:acquire`; missing assets may be acquired only when both scopes are present.
...
| 17-18: DataManager, API and frontend states | additive safe API, structured missing, HTTP/idempotency semantics, state separation, protected business-command entrypoints, immediate consumer handles, nullable asset request handles | 7, 8, 9 |
