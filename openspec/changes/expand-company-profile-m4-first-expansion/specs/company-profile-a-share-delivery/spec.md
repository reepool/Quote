...
Meeting the 4.1 numeric gates on the current two-report v4 sample MUST NOT be treated as scale-quality success or production authorization. A first expansion MUST be a separately reviewed change with an immutable a-priori plan that binds its preselected sample before enqueue. The current two-company live-run and source-review v1 files MUST remain on disk as the retained baseline of 2 independently reviewed reports and 2 occupied strata. New observations MUST use independent snapshots. `scale_quality_claim_allowed` MUST remain false until a later explicit authorization.
