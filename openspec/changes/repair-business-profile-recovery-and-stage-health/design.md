...
2. During enqueue, supersede non-running, non-completed work for the same frontier and policy when its processing identity differs from the current work. The row and original error remain available for audit; only its queue disposition changes. Running work is never superseded underneath an active lease.
