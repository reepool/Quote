...
- [ ] 1.2 基于已批准制造/材料 Gold 建立一份 `research_fixture` 参考画像输入和期望读取投影，覆盖业务概览、产品收支利、经营量、匿名关系、coverage、证据以及商品/供应链未授权状态。
...
- [ ] 2.2 实现严格的基础枚举和值对象：object type、subject scope、assertion class、requirement level、coverage status、period semantics、v1 action、metric type、capacity kind、comparison basis 和 disposition。
...
- [ ] 2.4 实现 BusinessOverview、Segment、Activity、Measurement、Relationship 和 BusinessEvent，并编码 Activity/Measurement 分离、source-native 保留、匿名身份、调整行、产能 kind、重述 basis 等已批准不变量。
- [ ] 2.5 实现 BusinessRegime、IndustryPackageAssignment 和 CoverageResult；保证 package/regime 绑定报告期，空集合不代表完成，五态 coverage 含确定性 reason/evidence。
- [ ] 2.6 从 Pydantic 模型生成稳定 JSON Schema，并加入 schema/version 序列化回归，避免手写第二套运行时 schema。

## 3. 研究员读取投影

- [ ] 3.1 实现纯函数投影，将通过结构/核验的对象按 overview、regime/package、segments、activities、measurements、inputs、counterparties、events、coverage 和 evidence index 分组。
...
- [ ] 4.1 实现单 chapter task 的 Report/Package/Task/Evidence request envelope；缺报告身份、active checklist、连续页、表头、单位或必要脚注时在 provider 调用前返回 typed preparation failure。
...
- [ ] 5.2 实现 deterministic-first 合并：完整结构化表格不调用 extract provider，未决语义才进入 fallback；确定性与模型候选通过同一 validator 和 physical occurrence 去重。
...
- [ ] 6.2 覆盖产品收入/成本/毛利率拆分、加工量/销量双锚、合并抵消、source-native 单位、主体 unclear、匿名关系、库存脚注和 PDF 页码正例。
- [ ] 6.3 覆盖产能缺 kind、重述缺 comparison basis、销量/销售额、库存量/存货金额、Activity 数值、第三方 actor、商品方向和完整产业链推断等阻塞反例。
