# company-profile-as-of-shenwan-l1-read Specification

## Purpose
The official company-profile registry resolves a Shenwan L1 name from the as-of history row's taxonomy version and official industry code by walking the active taxonomy parent chain. It keeps a top-level `sw_l1_name`, does not read a nested L1 name, leaves a broken chain as `other`, and does not replace that history with a later current membership. Production stays `not_authorized`.
