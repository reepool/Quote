# manufacturing-materials-stage4-minimum-slice Specification

## Purpose
The stage-4 minimum slice observes whether `extract_material_inputs` holds on three official 2025 annual reports, `300750.SZ`, `603659.SH`, and `920015.BJ`, across SZSE, SSE, and BSE. The run is research-only and stays at `accepted_for_review`. The closed observation is source recall 19/23, source accuracy 19/19, critical numeric errors 0, and `expansion_gates_met=false`. That false gate does not authorize the next expansion, a scale-quality claim, or production, and it does not overwrite the fixed two-company history.
