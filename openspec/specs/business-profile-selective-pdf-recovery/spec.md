# business-profile-selective-pdf-recovery Specification

## Purpose
Historical business-profile PDF recovery contract, retained as implementation reference only.

## Historical scope — retired from new profile execution
The requirements below describe the legacy recovery pipeline. They MUST NOT define the new all-A-share task state machine, field-family activation, whole-chapter acceptance or production enablement. Current task semantics follow company-profile-a-share-delivery and company-profile-bounded-semantic-workflow; shared PDF interfaces remain owned by shared-pdf-processing and shared-pdf-page-recovery-contract. Native-first reading, bounded OCR and reusable page caches may be reused where compatible, without reinstating legacy profile gates. This scope clarification changes no parser or runtime code.
## Requirements
