...
def test_valuation_input_sync_does_not_reuse_future_available_input(tmp_path):
    research_config = _build_research_config(tmp_path)
    storage = ResearchStorageManager(research_config)
    storage.initialize()
    storage.upsert_valuation_input(
    ...
    assert result["total_covered_instruments"] == 1
    assert result["total_missing_instruments"] == 1
    exchange = result["exchanges"][0]
    assert exchange["existing_covered_instruments"] == 0
    assert exchange["missing_instrument_ids"] == ["600000.SH"]
