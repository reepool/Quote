...
def test_company_profile_shadow_sync_task_calls_data_manager_and_clears_active_flag(monkeypatch):
    ...
    data_manager = Mock()
    data_manager.run_company_profile_shadow_sync = AsyncMock(
        return_value={
            "status": "success",
    ...
    result = asyncio.run(
        task.company_profile_shadow_sync(
            exchanges=["SSE"],
    ...
    data_manager.run_company_profile_shadow_sync.assert_awaited_once_with(
        exchanges=["SSE"],
        limit_per_exchange=10,
        budget_mode="balanced",
        allow_paid_proxy=False,
    ...
    assert "company_profile_shadow_sync" not in task._active_tasks
