...
def test_reader_lease_and_deletion_claim_are_mutually_fenced(tmp_path):
    config = _config(tmp_path)
    repository = AnnouncementAssetRepository(tmp_path / "research.db")
    repository.initialize_schema()
    store = ContentAddressedBlobStore(config)
    ...
    blob = repository.get_blob(asset.content_hash)
    assert blob is not None
    assert repository.release_retention_pin_by_key(
        blob_hash=asset.content_hash,
        pin_type="effective_asset",
        pin_key=asset.asset_id,
    )
    ...
    with repository.transaction() as conn:
        conn.execute(
            """INSERT INTO official_asset_deletion_intents(
                   ...
                   predecessor_asset_id, status, reason, planned_at, updated_at
               ) VALUES('reader-race-delete', 'official_asset_deletion_intent.v3',
                        ?, ?, ?, 'planned', 'test', ?, ?)""",
            (asset.content_hash, blob.canonical_path, asset.asset_id, now, now),
        )
    access = AnnouncementAssetAccess(
        repository=repository,
        config=config,
    ...
    assert not repository.claim_deletion(
        "reader-race-delete",
        lease_owner="deletion-worker",
        lease_expires_at=deletion_expiry,
        actor="test",
    ...
    with pytest.raises(RuntimeError, match="deletion is in progress"):
        repository.acquire_read_lease(
            blob_hash=asset.content_hash,
            owner="late-reader",
            ttl_seconds=30,
        )
...
def test_physically_deleted_predecessor_is_not_recreated_by_exact_ensure(tmp_path):
    config = _config(tmp_path)
    repository = AnnouncementAssetRepository(tmp_path / "research.db")
    repository.initialize_schema()
    store = ContentAddressedBlobStore(config)
    ...
    assert service.acquire_attachment(correction.attachment_id) is not None
    original_blob = repository.get_blob(original_asset.content_hash)
    assert original_blob is not None
    Path(original_blob.canonical_path).unlink()
    calls_before = retriever.calls
    request = EnsureRequest(
        source="cninfo",
        source_announcement_id="600000-2025-annual",
    ...
    public = access.ensure(request)
    assert public["reason_code"] == "non_effective_exact_filing_content_unavailable"
    assert public["asset_request_id"] is None
    with pytest.raises(FileNotFoundError):
        access.exact_observation_handle(request, authorized=True)
    assert retriever.calls == calls_before
