...
def test_master_governance_dry_run_simulates_auto_promoted_unknown_varieties(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        OfficialFuturesMarketDataProvider,
        "fetch_exchange_contract_bars_sync",
        lambda self, exchange, trade_date: [
            _official_contract_row(
                exchange=exchange,
                trade_date=trade_date,
                variety="BZ",
    ...
    assert result["counts"]["contracts_discovered"] == 1
    assert result["counts"]["would_write_contracts"] == 1
    assert result["contracts"][0]["contract_id"] == "CNF.BZ.DCE.BZ2601"
...
def test_dce_routes_exhausted_with_nested_timeout_is_non_retryable():
    classification = classify_official_futures_failure(
        "official DCE anti-bot challenge routes exhausted after bounded "
        "browser/proxy attempts; last_error=browser operation timed out"
    )
...
def test_dce_browser_refreshes_validated_proxy_before_acquiring_another_lease(monkeypatch):
    state = {"starts": 0, "proxy_gets": 0, "business_calls": 0, "leases": 0}
    ...
    class FakePage:
        ...
        async def evaluate(self, script, await_promise=False, return_by_value=False):
            ...
            state["business_calls"] += 1
            if state["business_calls"] == 2:
                return json.dumps(
                    {"status": -1, "ok": False, "text": "TypeError: Failed to fetch"}
                )
            return json.dumps(
                {
                    "status": 200,
    ...
    try:
        assert client.fetch_day_quotes_payload("2026-08-17")["success"] is True
        assert client.fetch_day_quotes_payload("2026-08-18")["success"] is True
    finally:
        client.close()

    assert state == {"starts": 2, "proxy_gets": 2, "business_calls": 3, "leases": 1}


def test_dce_browser_opens_fresh_proxy_recovery_cycle_with_run_wide_cap(monkeypatch):
    state = {"starts": 0, "business_calls": {}, "leases": 0}
    ...
    try:
        assert client.fetch_day_quotes_payload("2026-08-17")["success"] is True
        assert client.fetch_day_quotes_payload("2026-08-18")["success"] is True
        with pytest.raises(OfficialFuturesSourceUnavailable, match="routes exhausted"):
...
def test_official_calendar_backfill_preserves_strong_existing_evidence(monkeypatch, tmp_path):
    config = _research_config(tmp_path)
    config.modules["commodity_market_data"]["trading_day_governance"] = {
        "enabled_exchanges": ["DCE"],
        "official_calendar_backfill": {
        ...
        [
            FuturesTradingCalendarDay(
                ...
                metadata={
                    ...
                    "classification_rule": "official_daily_rows",
                    "row_count": 232,
                },
            )
        ]
    ...
    def _probe(self, exchange, trade_date):
        return OfficialFuturesDailyProbeResult(
            ...
            parser_version="fixture.v1",
            failure_reason=(
                "official DCE routes exhausted; "
                "last_error=browser operation timed out"
            ),
            metadata={
    ...
    assert result["status"] == "blocked"
    assert result["totals"]["preserved_verified"] == 1
    assert result["totals"]["unresolved_dates"] == 1
    assert exchange_result["preserved_verified_dates"] == ["2024-06-03"]
    assert exchange_result["failure_samples"][0]["trade_date"] == "2024-06-04"
    rows = storage.list_calendar_days(
        exchange="DCE",
        start_date="2024-06-03",
        end_date="2024-06-04",
    )
    assert len(rows) == 1
    assert rows[0]["metadata"]["classification_rule"] == "official_daily_rows"
...
def test_futures_id_helpers_are_deterministic():
    ...
    assert infer_contract_month("TC511", exchange="CZCE", trade_date="2015-10-15") == "2015-11"
    assert infer_contract_month("TA409", exchange="CZCE", trade_date="2024-06-03") == "2024-09"


def test_czce_three_digit_contract_month_uses_trade_date_context():
    cases = [
        ("SA306", "2022-06-16", "2023-06"),
        ("SH406", "2023-09-15", "2024-06"),
        ("FG612", "2015-12-15", "2016-12"),
        ("MA110", "2020-10-23", "2021-10"),
        ("CF609", "2015-10-15", "2016-09"),
        ("ZC901", "2018-01-09", "2019-01"),
        ("SH610", "2025-10-23", "2026-10"),
        ("SF908", "2018-08-15", "2019-08"),
        ("UR006", "2019-08-09", "2020-06"),
        ("PF611", "2025-11-17", "2026-11"),
        ("FG411", "2023-11-15", "2024-11"),
        ("PL601", "2025-07-22", "2026-01"),
        ("TA107", "2020-07-15", "2021-07"),
        ("CY907", "2018-07-16", "2019-07"),
        ("ZC211", "2021-11-08", "2022-11"),
        ("PM009", "2019-09-17", "2020-09"),
        ("SM308", "2022-08-15", "2023-08"),
        ("PF403", "2023-03-15", "2024-03"),
        ("CF707", "2016-07-15", "2017-07"),
        ("FG312", "2022-12-15", "2023-12"),
        ("TC602", "2015-10-15", "2016-02"),
        ("SF606", "2015-10-15", "2016-06"),
        ("PR506", "2024-08-30", "2025-06"),
        ("RS509", "2024-09-18", "2025-09"),
        ("FG602", "2015-10-15", "2016-02"),
        ("CF703", "2016-03-15", "2017-03"),
        ("OI405", "2023-05-18", "2024-05"),
        ("SR709", "2016-03-15", "2017-09"),
        ("ZC808", "2017-08-08", "2018-08"),
        ("MA903", "2018-03-15", "2019-03"),
        ("LR801", "2017-01-17", "2018-01"),
        ("SA210", "2021-10-22", "2022-10"),
        ("PF508", "2024-08-15", "2025-08"),
        ("CF903", "2018-03-15", "2019-03"),
        ("RM207", "2021-07-15", "2022-07"),
        ("RI307", "2022-07-15", "2023-07"),
        ("WH607", "2015-10-15", "2016-07"),
        ("CF103", "2020-03-16", "2021-03"),
        ("TA006", "2019-06-18", "2020-06"),
        ("TA310", "2022-10-24", "2023-10"),
        ("RI911", "2018-11-15", "2019-11"),
        ("AP204", "2021-04-16", "2022-04"),
        ("CF807", "2017-07-17", "2018-07"),
        ("PF410", "2023-10-23", "2024-10"),
        ("RM403", "2023-03-15", "2024-03"),
        ("OI305", "2022-05-19", "2023-05"),
        ("UR001", "2019-08-09", "2020-01"),
        ("RM203", "2021-03-15", "2022-03"),
        ("SA607", "2025-07-15", "2026-07"),
        ("UR101", "2020-01-16", "2021-01"),
    ]

    for code, trade_date, expected_month in cases:
        assert infer_contract_month(code, exchange="CZCE", trade_date=trade_date) == expected_month
