...
def test_status_summary_groups_tasks_by_status_and_domain():
    ...
    assert text.index("*行情与主数据*") < text.index("`daily_data_update`")
    assert text.index("*股东与披露*") < text.index("`shareholder_reconciliation_sync`")
...
def test_status_summary_accepts_task_status_enum():
    ...
    assert "手工/暂停 `1`" in text
    assert "*港美市场*" in text
    assert "`hkex_instrument_master_sync`" in text
