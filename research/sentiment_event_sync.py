"""
Sentiment event shadow sync service.
"""
...
from research.providers import SentimentEventProviderRegistry
...
class SentimentEventExchangeSyncResult:
    """Per-exchange result for sentiment event shadow sync."""
...
class SentimentEventShadowSyncService:
    """Run sentiment event shadow sync into research.db."""
    ...
    def __init__(
        ...
        *,
        ...
        registry: Optional[SentimentEventProviderRegistry] = None,
    ):
        self.db_ops = db_ops
        ...
        self.registry = registry or SentimentEventProviderRegistry(
    ...
    async def sync(
        ...
        *,
        ...
        results: List[SentimentEventExchangeSyncResult] = []
        ...
        total_written = sum(result.events_written for result in results)
    ...
    async def _sync_exchange(
        ...
        *,
    ...
    ) -> SentimentEventExchangeSyncResult:
        ...
        if not stock_instruments:
            return SentimentEventExchangeSyncResult(
                exchange=exchange,
                status="skipped",
        ...
        try:
            ...
            for candidate in plan.candidates:
                ...
                try:
                    snapshots = await provider.fetch_sentiment_events(
                    ...
                    dm_logger.warning(
                        "[SentimentEventSync] Provider %s (%s) failed for %s: %s",
                        candidate.source,
                        candidate.mode,
                        exchange,
                        e,
                ...
                for snapshot in snapshots:
                    self.storage.upsert_sentiment_event(snapshot, ingestion_run_id=run_id)
            ...
            self.storage.finish_ingestion_run(
                ...
                rows_written=0,
                error_message=None if optional_empty_exchange else "No provider returned sentiment events",
                metadata={
                    "exchange": exchange,
                    "attempted_sources": attempted_sources,
