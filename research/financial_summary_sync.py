"""
Financial summary shadow sync service.
"""
...
from research.providers import FinancialSummaryProviderRegistry
...
class FinancialSummaryExchangeSyncResult:
    """Per-exchange result for financial summary shadow sync."""
...
class FinancialSummaryShadowSyncService:
    """Run financial summary shadow sync into research.db."""
    ...
    def __init__(
        ...
        *,
        ...
        registry: Optional[FinancialSummaryProviderRegistry] = None,
    ):
        self.db_ops = db_ops
        ...
        self.registry = registry or FinancialSummaryProviderRegistry(
    ...
    async def sync(
        ...
        *,
        ...
        results: List[FinancialSummaryExchangeSyncResult] = []
    ...
    async def _sync_exchange(
        ...
        *,
    ...
    ) -> FinancialSummaryExchangeSyncResult:
        ...
        if not stock_instruments:
            return FinancialSummaryExchangeSyncResult(
                exchange=exchange,
                status="skipped",
        ...
        try:
            ...
            for candidate in plan.candidates:
                ...
                try:
                    snapshots = await provider.fetch_financial_summaries(
                    ...
                    dm_logger.warning(
                        "[FinancialSummarySync] Provider %s (%s) failed for %s: %s",
                        candidate.source,
                        candidate.mode,
                        exchange,
                        e,
                ...
                for snapshot in snapshots:
                    self.storage.upsert_financial_summary(snapshot, ingestion_run_id=run_id)
                ...
                return FinancialSummaryExchangeSyncResult(
                    ...
                    mode=candidate.mode,
                    attempted_sources=attempted_sources,
                    summaries_written=len(snapshots),
                )
            ...
            self.storage.finish_ingestion_run(
                ...
                rows_written=0,
                error_message=None if optional_empty_exchange else "No provider returned financial summaries",
                metadata={
                    "exchange": exchange,
                    "attempted_sources": attempted_sources,
