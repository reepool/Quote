"""
Analyst forecast shadow sync service.
"""
...
from research.providers import AnalystForecastProviderRegistry
...
class AnalystForecastExchangeSyncResult:
    """Per-exchange result for analyst forecast shadow sync."""
...
class AnalystForecastShadowSyncService:
    """Run analyst forecast shadow sync into research.db."""
    ...
    def __init__(
        ...
        *,
        ...
        registry: Optional[AnalystForecastProviderRegistry] = None,
    ):
        self.db_ops = db_ops
        ...
        self.registry = registry or AnalystForecastProviderRegistry(
    ...
    async def sync(
        ...
        *,
        ...
        results: List[AnalystForecastExchangeSyncResult] = []
        ...
        total_written = sum(result.forecasts_written for result in results)
    ...
    async def _sync_exchange(
        ...
        *,
    ...
    ) -> AnalystForecastExchangeSyncResult:
        ...
        if not stock_instruments:
            return AnalystForecastExchangeSyncResult(
                exchange=exchange,
                status="skipped",
        ...
        try:
            ...
            for candidate in plan.candidates:
                ...
                try:
                    snapshots = await provider.fetch_analyst_forecasts(
                    ...
                    dm_logger.warning(
                        "[AnalystForecastSync] Provider %s (%s) failed for %s: %s",
                        candidate.source,
                        candidate.mode,
                        exchange,
                        e,
                ...
                for snapshot in snapshots:
                    self.storage.upsert_analyst_forecast(snapshot, ingestion_run_id=run_id)
                ...
                return AnalystForecastExchangeSyncResult(
                    ...
                    mode=candidate.mode,
                    attempted_sources=attempted_sources,
                    forecasts_written=len(snapshots),
                )
            ...
            self.storage.finish_ingestion_run(
                ...
                rows_written=0,
                error_message=None if optional_empty_exchange else "No provider returned analyst forecasts",
                metadata={
                    "exchange": exchange,
                    "attempted_sources": attempted_sources,
