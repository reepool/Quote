...
from research.providers import IndustryProviderRegistry
...
class IndustryExchangeSyncResult:
    """Per-exchange result for industry shadow sync."""
...
class IndustryShadowSyncService:
    """Run industry shadow sync into research.db."""
    ...
    def __init__(
        ...
        *,
        ...
        registry: Optional[IndustryProviderRegistry] = None,
    ):
        self.db_ops = db_ops
        ...
        self.registry = registry or IndustryProviderRegistry(research_config=self.research_config)
    ...
    async def sync(
        ...
        *,
        ...
        results: List[IndustryExchangeSyncResult] = []
        ...
        total_written = sum(result.memberships_written for result in results)
        success_count = sum(1 for result in results if result.status == "success")

        return {
            "status": "success" if success_count else "degraded",
            "exchanges": [asdict(result) for result in results],
            "total_memberships_written": total_written,
    ...
    async def _sync_exchange(
        ...
        *,
        ...
        if not stock_instruments:
            return IndustryExchangeSyncResult(
                exchange=exchange,
                status="skipped",
        ...
        try:
            ...
            for candidate in plan.candidates:
                ...
                try:
                    snapshots = await provider.fetch_industries(
                    ...
                    dm_logger.warning(
                        "[IndustrySync] Provider %s (%s) failed for %s: %s",
                        candidate.source,
                        candidate.mode,
                        exchange,
                        e,
                ...
                for snapshot in snapshots:
                    self.storage.upsert_industry_membership(snapshot, ingestion_run_id=run_id)
                ...
                return IndustryExchangeSyncResult(
                    ...
                    mode=candidate.mode,
                    attempted_sources=attempted_sources,
                    memberships_written=len(snapshots),
                )
            ...
            self.storage.finish_ingestion_run(
                ...
                rows_written=0,
                error_message=None if optional_empty_exchange else "No provider returned industry memberships",
                metadata={
                    "exchange": exchange,
                    "attempted_sources": attempted_sources,
